...
def test_continuity_validation():
    ...
    try:
        ...
        print("  ERROR: Discontinuous curve was created! This should have failed.")

    except ValueError as e:
        print("  ✅ Good! CompositeCurve properly rejected discontinuous segments:")
        print(f"     {e}")
    ...
    try:
        ...
        print(
            f"  ✅ Success! Continuous curve created with {len(continuous.segments)} segments"
        )
    ...
    try:
        ...
        seg2_mask = lambda x_val, y_val: (1.001 <= x_val <= 2) and (
            -0.1 <= y_val <= 0.1
        )
        seg2 = TrimmedImplicitCurve(
            seg2_line, seg2_mask, endpoints=[(1.001, 0), (2, 0)]
        )

        # Should fail with default tolerance (1e-6)
        try:
            strict = CompositeCurve([seg1, seg2])
            print("  ❌ Strict tolerance should have failed")
        except ValueError:
            print("  ✅ Strict tolerance (1e-6) properly rejected gap of 0.001")

        # Should succeed with relaxed tolerance
        try:
            relaxed = CompositeCurve(
                [seg1, seg2], validate_continuity=True, continuity_tolerance=0.01
            )
            print("  ✅ Relaxed tolerance (0.01) accepted gap of 0.001")
        except ValueError as e:
    ...
    try:
        ...
        print("  ✅ Validation bypass worked - discontinuous curve created")
        print("     (This is allowed when validate_continuity=False)")
