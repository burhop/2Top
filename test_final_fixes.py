...
from geometry.factories import (
    create_ellipse_parabola_hybrid, create_superellipse_circle_hybrid, 
    create_lens_shape, create_heart_shape
)
...
def test_final_fixes():
    ...
    for name, creator_func, color in shapes:
        try:
                ...
                if hasattr(segment, 'base_curve') and hasattr(segment, 'mask'):
                    ...
                    for row in range(X_shifted.shape[0]):
                        for col in range(X_shifted.shape[1]):
                            mask_grid[row, col] = segment.mask(X_shifted[row, col], Y_shifted[row, col])
            ...
            # Add label
            ax.text(offset_x, offset_y + 1.8, name, ha='center', va='center', 
    ...
    ax.set_title('Fixed Composite Shapes\n(Egg shape now properly oriented)', fontsize=14, fontweight='bold')
    ...
    print(f"  💾 Final test plot saved as final_fixes_test.png")
