...
from geometry.parameter_interface import (
    CircleParameters,
    RectangleParameters,
)
...
def demo_single_parameter_animation():
    ...
    try:
        ...
        for cache_id, info in cache_info.items():
            print(
                f"  {cache_id}: {info['frame_count']} frames, {info['cache_size_mb']:.2f} MB"
            )
...
def demo_multi_parameter_animation():
    ...
    rect_heights = (
        1.0 + 0.6 * np.sin(np.linspace(np.pi / 2, 2 * np.pi + np.pi / 2, frame_count))
    ).tolist()
...
def demo_dependency_tracking():
    ...
    print(
        f"Dependent radius after update: {scene.get_parameter('dependent', 'radius')}"
    )
...
def demo_animation_cache_management():
    ...
    for cache_id, info in cache_info.items():
        print(
            f"  {cache_id}: {info['frame_count']} frames, "
