...
def main():
    ...
    proc = ProceduralCurve(
        lambda X, Y: (X - 0.5) ** 2 + (Y + 0.25) ** 2 - 0.6, variables=(x, y)
    )
    ...
    print(
        "poly.contains(1.0,0.3, region)=",
        poly.contains(1.0, 0.3, region_containment=True),
    )
    print(
        "poly.contains(1.0,0.0, region)=",
        poly.contains(1.0, 0.0, region_containment=True),
    )
    print(
        "poly.contains(3.0,3.0, region)=",
        poly.contains(3.0, 3.0, region_containment=True),
    )

    # Fields
    sdf = SignedDistanceStrategy(resolution=0.1).generate_field(region)
    occ = OccupancyFillStrategy(inside_value=1.0, outside_value=0.0).generate_field(
        region
    )
