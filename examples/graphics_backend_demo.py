...
def demo_graphics_backend_interface():
    ...
    rectangle = factory.create_rectangle_region(
        (2, 1.25), (4, 2.75)
    )  # center (3,2), width 2.0, height 1.5
    ...
    for obj_id, data in field_data.items():
        if data.get("data") is not None:
            ...
            print(
                f"{obj_id}: {stats['finite_count']}/{stats['total_count']} finite values"
            )
            print(
    ...
    for obj_id, data in region_data.items():
        if data.get("inside_mask") is not None:
            stats = data["statistics"]
            print(
                f"{obj_id}: {stats['inside_count']} inside points ({stats['inside_percentage']:.1f}%)"
            )
            print(
                f"  Boundary: {stats['boundary_count']} points ({stats['boundary_percentage']:.1f}%)"
            )
        else:
...
def demo_mcp_command_handler():
    ...
    for category, cmd_list in sorted(categories.items()):
        print(
            f"  {category}: {len(cmd_list)} commands ({', '.join(cmd_list[:3])}{'...' if len(cmd_list) > 3 else ''})"
        )
    ...
    # Modify object style
    result = mcp_handler.handle_command(
        "object.set_style",
        {
            "object_id": "cmd_circle",
    ...
    result = mcp_handler.handle_command(
        "scene.get_object_info", {"object_id": "cmd_circle"}
    )
    if result["success"]:
        info = result["data"]
    ...
    # Register dependency
    result = mcp_handler.handle_command(
        "dependency.register", {"source_id": "cmd_circle", "dependent_id": "cmd_rect"}
    )
    if result["success"]:
        print("✓ Registered dependency: rectangle depends on circle")

    # Get dependencies
    result = mcp_handler.handle_command("dependency.get", {"object_id": "cmd_circle"})
    if result["success"]:
    ...
    # Get scene bounds
    result = mcp_handler.handle_command("render.get_scene_bounds", {"padding": 0.15})
    if result["success"]:
        bounds = result["data"]
    ...
    # Get curve paths
    result = mcp_handler.handle_command(
        "render.get_curve_paths", {"bounds": bounds, "resolution": 80}
    )
    if result["success"]:
    ...
    # Get region data
    result = mcp_handler.handle_command(
        "render.get_region_data", {"bounds": bounds, "resolution": [40, 40]}
    )
    if result["success"]:
    ...
    result = mcp_handler.handle_command(
        "render.scene_image",
        {"bounds": bounds, "resolution": [600, 400], "show_grid": True},
    )
    if result["success"]:
    ...
    try:
        result = mcp_handler.handle_command("scene.save", {"filename": scene_file})
        if result["success"]:
        ...
        if result["success"]:
            print(
                f"✓ Cleared scene ({result['data']['cleared_objects']} objects removed)"
            )

        # Load scene back
        result = mcp_handler.handle_command("scene.load", {"filename": scene_file})
        if result["success"]:
            print("✓ Loaded scene back from file")
...
def demo_error_handling():
    ...
    print("-" * 40)
    result = mcp_handler.handle_command(
        "object.get_style", {"object_id": "nonexistent_object"}
    )
    print(f"Result: success={result['success']}")
