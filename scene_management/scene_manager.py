...
class SceneManager:
    ...
    def __init__(self):
        ...
        self._dependency_descriptions: Dict[
            str, Dict[str, str]
        ] = {}  # dependent_id -> {source_id: description}
    ...
    def clear(self) -> None:
        ...
        self._dependency_descriptions.clear()
    ...
    def add_object(
        self,
        obj_id: str,
        obj: Union[ImplicitCurve, CompositeCurve, AreaRegion],
    ...
    def update_parameter(
        self, obj_id: str, parameter: str, value: Any, update_dependents: bool = True
    ) -> None:
        """
        ...
        if not hasattr(obj, "set_parameter"):
            raise AttributeError(
                f"Object '{obj_id}' does not support parameter updates"
            )
    ...
    def list_parameters(self, obj_id: str) -> List[str]:
        ...
        if not hasattr(obj, "list_parameters"):
            raise AttributeError(
                f"Object '{obj_id}' does not support parameter listing"
            )
    ...
    def register_dependency(
        ...
        Args:
            ...
            description: Optional descriptive text explaining the relationship/anchor
        """
        ...
        # Add dependency description if provided
        if description:
            if dependent_id not in self._dependency_descriptions:
                self._dependency_descriptions[dependent_id] = {}
            self._dependency_descriptions[dependent_id][source_id] = description
    ...
    def _remove_dependencies(self, obj_id: str) -> None:
        ...
        # Clean up dependency descriptions
        if obj_id in self._dependency_descriptions:
            del self._dependency_descriptions[obj_id]
        for dep_id in list(self._dependency_descriptions.keys()):
            if obj_id in self._dependency_descriptions[dep_id]:
                del self._dependency_descriptions[dep_id][obj_id]
                if not self._dependency_descriptions[dep_id]:
                    del self._dependency_descriptions[dep_id]
    ...
    def create_parameter_animation(
        ...
        try:
            ...
            # Create animation
            self._create_animation_from_frames(
                frame_files if cache_frames else frames, filename, fps
            )
            ...
            if cache_frames:
                self._animation_cache[cache_id] = {
                    ...
                    "cache_size_mb": sum(os.path.getsize(f) for f in frame_files)
                    / (1024 * 1024),
                }
            ...
            self.update_parameter(
                obj_id, parameter, original_value, update_dependents=True
            )

    def create_multi_parameter_animation(
        self,
        ...
        if cache_frames and cache_id in self._animation_cache:
            print("Using cached multi-parameter animation")
            self.replay_cached_animation(cache_id, filename, fps)
            return
        ...
        try:
            for frame_idx in range(frame_count):
                ...
                for anim in animations:
                    ...
                    self.update_parameter(
                        obj_id, parameter, value, update_dependents=True
                    )
                ...
                if cache_frames:
                    frame_file = (
                        self._cache_directory / f"{cache_id}_frame_{frame_idx:04d}.png"
                    )
                    fig.savefig(frame_file, dpi=100, bbox_inches="tight")
                    frame_files.append(str(frame_file))
    ...
    def replay_cached_animation(
        self, cache_id: str, filename: str, fps: int = 10
    ) -> None:
    ...
    def _generate_multi_cache_id(
        self, animations: List[Dict], resolution: Tuple[int, int]
    ) -> str:
        """Generate cache ID for multi-parameter animation."""
    ...
    def save_scene(self, filename: str) -> None:
        ...
        scene_data = {
            ...
            "dependency_descriptions": self._dependency_descriptions.copy(),
    ...
    def load_scene(self, filename: str) -> None:
        ...
        if isinstance(styles_data, dict):
            ...
            print("Warning: Invalid styles data type, using empty dict")
            self._styles = {}
        ...
        if isinstance(groups_data, dict):
            ...
            print("Warning: Invalid groups data type, using empty dict")
            self._groups = {}
        ...
        if isinstance(deps_data, dict):
            ...
            print("Warning: Invalid dependencies data type, using empty dict")
            self._dependencies = {}
        ...
        if isinstance(reverse_deps_data, dict):
            ...
            print("Warning: Invalid reverse_dependencies data type, using empty dict")
            self._reverse_dependencies = {}

        dep_desc_data = scene_data.get("dependency_descriptions", {})
        if isinstance(dep_desc_data, dict):
            self._dependency_descriptions = dep_desc_data
        else:
        ...
        if isinstance(objects_data, dict):
            for obj_id, obj_data in objects_data.items():
                try:
                    if isinstance(obj_data, dict):
                        print(
                            f"Loading object '{obj_id}' of type '{obj_data.get('type')}'"
                        )
                        obj_type = obj_data.get("type")
                        obj = None

                        # Try to deserialize using curve dispatcher first
                        try:
                            from geometry.implicit_curve import ImplicitCurve

                            obj = ImplicitCurve.from_dict(obj_data)
                        except Exception:
                            pass

                        # If not curve, try field/region types
                        if obj is None:
                            if obj_type == "CurveField":
                                from geometry.base_field import CurveField

                                obj = CurveField.from_dict(obj_data)
                            elif obj_type == "BlendedField":
                                from geometry.base_field import BlendedField

                                obj = BlendedField.from_dict(obj_data)
                            elif obj_type == "SignedDistanceField":
                                from geometry.field_strategy import SignedDistanceField

                                obj = SignedDistanceField.from_dict(obj_data)
                            elif obj_type == "OccupancyField":
                                from geometry.field_strategy import OccupancyField

                                obj = OccupancyField.from_dict(obj_data)
                            elif obj_type == "AreaRegion":
                                from geometry.area_region import AreaRegion

                                obj = AreaRegion.from_dict(obj_data)

                        if obj is not None:
                            self._objects[obj_id] = obj
                        else:
                            print(
                                f"Warning: Unknown object type or failed to reconstruct object '{obj_id}'"
                            )
                    else:
            ...
            print("Warning: Invalid objects data type, skipping object loading")
