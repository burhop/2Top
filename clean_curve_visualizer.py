...
class CleanCurveVisualizerApp:
    ...
    def __init__(self, root):
        ...
        self.detect_overlap = tk.BooleanVar(
            value=False
    ...
    def setup_controls(self, parent):
        ...
        # Title
        ttk.Label(
            parent, text="Clean Curve Visualizer", font=("Arial", 14, "bold")
        ).pack(pady=(0, 10))
        ...
        ttk.Button(add_frame1, text="Circle", command=self.add_circle).pack(
            side=tk.LEFT, padx=1
        )
        ttk.Button(add_frame1, text="Line", command=self.add_line).pack(
            side=tk.LEFT, padx=1
        )
        ttk.Button(add_frame1, text="Ellipse", command=self.add_ellipse).pack(
            side=tk.LEFT, padx=1
        )
        ...
        ttk.Button(add_frame2, text="Parabola", command=self.add_parabola).pack(
            side=tk.LEFT, padx=1
        )
        ttk.Button(add_frame2, text="Hyperbola", command=self.add_hyperbola).pack(
            side=tk.LEFT, padx=1
        )
        ttk.Button(add_frame2, text="Superellipse", command=self.add_superellipse).pack(
            side=tk.LEFT, padx=1
        )
        ...
        ttk.Button(add_frame3, text="Trimmed", command=self.add_trimmed).pack(
            side=tk.LEFT, padx=1
        )
        ttk.Button(add_frame3, text="Square", command=self.add_square).pack(
            side=tk.LEFT, padx=1
        )
        ttk.Button(
            add_frame3, text="Mixed Composites", command=self.add_mixed_composite
        ...
        ttk.Button(add_frame4, text="Test Squares", command=self.test_squares).pack(
            side=tk.LEFT, padx=1
        )
        ttk.Button(
            add_frame4, text="Test Figure-8", command=self.test_figure_eight
        ).pack(side=tk.LEFT, padx=1)
        ttk.Button(
            add_frame4, text="Working Composites", command=self.test_working_composites
        ).pack(side=tk.LEFT, padx=1)
        ...
        ttk.Label(custom_frame, text="Custom f(x,y)=0:", font=("Arial", 8)).pack(
            anchor=tk.W
        )
        self.custom_expr = tk.StringVar(value="x**3 + y**3 - 1")
        custom_entry = ttk.Entry(
            custom_frame, textvariable=self.custom_expr, font=("Courier", 8)
        )
        custom_entry.pack(fill=tk.X, pady=1)
        custom_entry.bind("<Return>", lambda e: self.add_custom())

        ttk.Button(custom_frame, text="Add Custom", command=self.add_custom).pack(
            pady=2
        )
        ...
        ttk.Button(
            intersect_frame,
            text="Find All Intersections",
            command=self.find_intersections,
        ).pack(pady=2)
        ...
        ttk.Label(preset_frame, text="Quick Tests:", font=("Arial", 8)).pack(
            anchor=tk.W
        )
        ...
        ttk.Button(
            preset_buttons, text="Circle+Line", command=self.preset_circle_line
        ).pack(side=tk.LEFT, padx=1)
        ttk.Button(
            preset_buttons, text="2 Circles", command=self.preset_two_circles
        ).pack(side=tk.LEFT, padx=1)
        ttk.Button(
            preset_buttons, text="Multi-Segment", command=self.preset_multi_segment
        ).pack(side=tk.LEFT, padx=1)
        ...
        ttk.Checkbutton(
            display_frame, text="Detect Overlap (slower)", variable=self.detect_overlap
        ).pack(anchor=tk.W, padx=5)
        ...
        ttk.Entry(settings_frame, textvariable=self.plot_range, width=8).pack(
            anchor=tk.W, padx=5, pady=2
        )

        ttk.Button(settings_frame, text="Update Plot", command=self.update_plot).pack(
            pady=5
        )
    ...
    def setup_plot(self, parent):
        ...
        self.ax.set_title(
            "Implicit Curves with Precise Intersections", fontweight="bold"
        )
    ...
    def add_superellipse(self):
        ...
        super_count = len(
            [c for c in self.curves if "like" in c[0] or "Rounded" in c[0]]
        )
    ...
    def add_mixed_composite(self):
        ...
        mixed_configs = [
            (lambda: self._create_circle_line_hybrid(), "D-Shape (Circle + Line)"),
            (
            ...
            (lambda: self._create_heart_shape(), "Heart Shape (Circles + Parabola)"),
            (lambda: self._create_lens_shape(), "Lens Shape (Two Circles)"),
        ]
    ...
    def add_composite(self):
        ...
        composite_configs = [
            ...
            (lambda: self._create_L_shape(), "L-Shape"),
            # T-shape (three line segments)
            (lambda: self._create_T_shape(), "T-Shape"),
            # Triangle (three line segments)
            (lambda: self._create_triangle(), "Triangle"),
            # House shape (square + triangle roof)
            (lambda: self._create_house_shape(), "House Shape"),
            # Zigzag pattern
            (lambda: self._create_zigzag_pattern(), "Zigzag Pattern"),
            # Staircase pattern
            (lambda: self._create_staircase(), "Staircase"),
            # Figure-eight
            (lambda: self._create_figure_eight(), "Figure Eight"),
            # Simple square (for comparison)
            (lambda: create_square_from_edges((-0.8, -0.8), (0.8, 0.8)), "Square"),
        ]
    ...
    def add_square(self):
        ...
        square_count = len(
            [c for c in self.curves if "Square" in c[0] or "Rectangle" in c[0]]
        )
        ...
        if square_count < len(square_configs):
            ...
            cx = np.random.uniform(-0.5, 0.5)
            cy = np.random.uniform(-0.5, 0.5)
            curve = create_square_from_edges(
    ...
    def find_intersections(self):
        ...
        for i in range(len(self.curves)):
            for j in range(i + 1, len(self.curves)):
                ...
                for point in intersections:
                    ...
                    if (
                        abs(val1) < 1e-6 and abs(val2) < 1e-6
                    ):  # Both curves pass through point
        ...
        if self.intersection_data:
            messagebox.showinfo(
                "Results", f"Found {len(self.intersection_data)} precise intersections"
            )
        else:
    ...
    def update_intersect_list(self):
        ...
        for i, data in enumerate(self.intersection_data):
            ...
            error_info = (
                f"(err: {max(error1, error2):.2e})"
                if max(error1, error2) > 1e-8
                else ""
            )

            self.intersect_listbox.insert(
                tk.END,
            ...
            for i, (name, curve) in enumerate(self.curves):
                try:
                    ...
                    if hasattr(curve, "base_curve") and hasattr(curve, "mask"):
                        ...
                        self.ax.contour(
                            X, Y, Z, levels=[0], colors=[color], linewidths=3
                        )
    ...
    def _create_L_shape(self):
        ...
        vertical_mask = lambda x_val, y_val: (-0.6 <= x_val <= -0.4) and (
            -1 <= y_val <= 0
        )
        ...
        horizontal_mask = lambda x_val, y_val: (-0.5 <= x_val <= 0.5) and (
            -1.1 <= y_val <= -0.9
        )
    ...
    def _create_triangle(self):
        ...
        bottom_mask = lambda x_val, y_val: (-1 <= x_val <= 1) and (
            -0.6 <= y_val <= -0.4
        )
        segments.append(
            TrimmedImplicitCurve(bottom, bottom_mask, endpoints=[(-1, -0.5), (1, -0.5)])
        )
        ...
        segments.append(
            TrimmedImplicitCurve(right, right_mask, endpoints=[(1, -0.5), (0, 1)])
        )
        ...
        segments.append(
            TrimmedImplicitCurve(left, left_mask, endpoints=[(0, 1), (-1, -0.5)])
        )
    ...
    def _create_house_shape(self):
        ...
        bottom_mask = lambda x_val, y_val: (-0.5 <= x_val <= 0.5) and (
            -1.1 <= y_val <= -0.9
        )
        segments.append(
        ...
        roof_right_mask = lambda x_val, y_val: (0 <= x_val <= 0.5) and (
            0 <= y_val <= 0.5
        )
        segments.append(
        ...
        roof_left_mask = lambda x_val, y_val: (-0.5 <= x_val <= 0) and (
            0 <= y_val <= 0.5
        )
        segments.append(
            TrimmedImplicitCurve(
                roof_left, roof_left_mask, endpoints=[(0, 0.5), (-0.5, 0)]
            )
        )
    ...
    def _create_staircase(self):
        ...
        step1_h_mask = lambda x_val, y_val: (-1 <= x_val <= -0.3) and (
            -0.6 <= y_val <= -0.4
        )
        segments.append(
            TrimmedImplicitCurve(
                step1_h, step1_h_mask, endpoints=[(-1, -0.5), (-0.3, -0.5)]
            )
        )

        # Vertical connector from (-0.3, -0.5) to (-0.3, 0)
        step1_v = PolynomialCurve(x + 0.3, (x, y))
        step1_v_mask = lambda x_val, y_val: (-0.4 <= x_val <= -0.2) and (
            -0.5 <= y_val <= 0
        )
        segments.append(
            TrimmedImplicitCurve(
                step1_v, step1_v_mask, endpoints=[(-0.3, -0.5), (-0.3, 0)]
            )
        )

        # Step 2: horizontal from (-0.3, 0) to (0.3, 0)
        step2_h = PolynomialCurve(y, (x, y))
        step2_h_mask = lambda x_val, y_val: (-0.3 <= x_val <= 0.3) and (
            -0.1 <= y_val <= 0.1
        )
        segments.append(
            TrimmedImplicitCurve(step2_h, step2_h_mask, endpoints=[(-0.3, 0), (0.3, 0)])
        )

        # Vertical connector 2 from (0.3, 0) to (0.3, 0.5)
        step2_v = PolynomialCurve(x - 0.3, (x, y))
        step2_v_mask = lambda x_val, y_val: (0.2 <= x_val <= 0.4) and (
            0 <= y_val <= 0.5
        )
        segments.append(
            TrimmedImplicitCurve(
                step2_v, step2_v_mask, endpoints=[(0.3, 0), (0.3, 0.5)]
            )
        )

        # Step 3: horizontal from (0.3, 0.5) to (1, 0.5)
        step3_h = PolynomialCurve(y - 0.5, (x, y))
        step3_h_mask = lambda x_val, y_val: (0.3 <= x_val <= 1) and (
            0.4 <= y_val <= 0.6
        )
        segments.append(
            TrimmedImplicitCurve(
                step3_h, step3_h_mask, endpoints=[(0.3, 0.5), (1, 0.5)]
            )
        )
    ...
    def test_working_composites(self):
        ...
        working_composites = [
            ...
            (
                lambda: create_square_from_edges((-1, -1), (1, 1)),
                "Unit Square",
                "black",
            ),
            (
                lambda: create_square_from_edges((-0.5, -0.5), (0.5, 0.5)),
                "Small Square",
                "gray",
            ),
        ]
        ...
        try:
            ...
            print("  ⚠️ Added Figure-Eight (may have gaps)")
            success_count += 1
        except Exception as e:
            print(f"  ❌ Failed to add Figure-Eight: {e}")
        ...
        try:
            ...
            print("  🔧 Added Heart Shape (recently fixed)")
            success_count += 1
        except Exception as e:
    ...
    def debug_single_square(self):
        ...
        try:
            ...
            for i, segment in enumerate(square.segments):
                ...
                print(
                    f"    Test point {test_point}: contains={contains}, mask={mask_result}, eval={eval_result:.3f}"
                )
