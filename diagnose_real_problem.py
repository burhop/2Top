    ...
    for i, segment in enumerate(heart.segments):
        ...
        if hasattr(segment, "base_curve"):
            ...
            for px, py in test_points:
                ...
                print(
                    f"    Point ({px}, {py}): mask={mask_result}, base_val={base_val:.3f}"
                )
    ...
    for seg_idx, test_points in test_cases:
        ...
        for px, py in test_points:
            ...
            if hasattr(segment, "base_curve"):
                ...
                print(
                    f"  Point ({px:4.1f}, {py:4.1f}): mask={mask_result:5}, base={base_val:6.3f}, segment={segment_val:6.3f}"
                )
            else:
                segment_val = segment.evaluate(px, py)
                print(
                    f"  Point ({px:4.1f}, {py:4.1f}): mask={mask_result:5}, segment={segment_val:6.3f}"
                )
