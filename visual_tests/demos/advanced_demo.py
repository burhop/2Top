...
def demo_trimmed_curves():
    ...
    fig, axes = plot_manager.setup_figure(
        2, 2, figsize=(12, 12), suptitle="Trimmed Curves Demo"
    )
    ...
    for i, (name, mask_func) in enumerate(masks):
        ...
        plot_manager.plot_curve_contour(
            ax,
            base_circle,
            X,
            Y,
...
def demo_composite_curves():
    ...
    # Set up the plot
    fig, axes = plot_manager.setup_figure(
        1, 2, figsize=(12, 6), suptitle="Composite Curves Demo"
    )
    ...
    for i, (start_angle, end_angle) in enumerate(angle_ranges):
        ...
        ax.plot(qx, qy, color=colors[i], linewidth=3, label=f"Quarter {i + 1}")
...
def demo_complex_regions():
    ...
    # Set up the plot
    fig, axes = plot_manager.setup_figure(
        1, 3, figsize=(15, 5), suptitle="Complex Regions Demo"
    )
    ...
    regions = [
        ...
        (
            "Complex Triangle",
            RegionFactory.create_triangle_region([(-2.5, -2), (2.5, -2), (0, 2.5)]),
        ),
        (
            "Wide Rectangle",
            RegionFactory.create_rectangle_region((-2.8, -1.2), (2.8, 1.2)),
        ),
    ]
...
def demo_advanced_analysis():
    ...
    triangle_region = RegionFactory.create_triangle_region(
        [(-2, -1.5), (2, -1.5), (0, 2)]
    )
    ...
    for curve_name, curve in [("Circle", circle), ("Ellipse", ellipse)]:
        ...
        print(
            f"    On-curve points: {sum(1 for r in results['points'] if r['on_curve'])}"
        )
    ...
    print("  High-resolution triangle analysis:")
    print(f"    Grid size: {focused_X.size} points")
    print(
        f"    Inside: {stats['inside_count']} points ({stats['inside_percentage']:.2f}%)"
    )
    print(
        f"    Boundary: {stats['boundary_count']} points ({stats['boundary_percentage']:.2f}%)"
    )
    print(
        f"    Boundary resolution: {stats['boundary_count'] / stats['total_points'] * 100:.3f}%"
    )
