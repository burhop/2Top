...
def test_basic_conic_sections():
    ...
    fig, axes = plot_manager.setup_figure(
        2, 2, figsize=(12, 12), suptitle="Basic Conic Sections"
    )
    ...
    for i, (name, curve, equation) in enumerate(curves):
        ...
        plot_manager.plot_curve_contour(
            ax, curve, X, Y, title=title, color="blue", show_levels=True
        )
...
def test_polynomial_curves():
    ...
    # Set up the plot
    fig, axes = plot_manager.setup_figure(
        2, 3, figsize=(15, 10), suptitle="Polynomial Curves"
    )
    ...
    curves = [
        ...
        (
            "Circle (Polynomial)",
            PolynomialCurve(x**2 + y**2 - 4, (x, y)),
            "x² + y² = 4",
        ),
        (
            "Lemniscate",
            PolynomialCurve((x**2 + y**2) ** 2 - 2 * (x**2 - y**2), (x, y)),
            "(x²+y²)² = 2(x²-y²)",
        ),
    ]
    ...
    for i, (name, curve, equation) in enumerate(curves):
        ...
        print(f"Plotting {name}...")
        plot_manager.plot_curve_contour(
            ax, curve, X, Y, title=title, color=color, show_levels=True
        )
