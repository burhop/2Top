...
def test_trimmed_curves():
    ...
    fig, axes = plot_manager.setup_figure(
        2, 3, figsize=(15, 10), suptitle="Trimmed Curves"
    )
    ...
    masks = [
        ...
        (
            "Upper Right Arc",
            lambda px, py: px >= 0 and py >= 0 and px**2 + py**2 <= 4.5,
        ),
    ...
    for i, (name, mask_func) in enumerate(masks):
        ...
        if mask_points_x:
            ax.plot(
                mask_points_x, mask_points_y, "b-", linewidth=3, label="Trimmed Curve"
            )
...
def test_composite_curves():
    ...
    # Set up the plot
    fig, axes = plot_manager.setup_figure(
        2, 2, figsize=(12, 12), suptitle="Composite Curves"
    )
    ...
    arc_mask = (
        lambda px, py: px >= 0 and py >= 0 and abs(circle_curve.evaluate(px, py)) < 0.1
    )
    ...
    segment_masks = [
        ...
        lambda px, py: 2 * np.pi / 3
        <= np.arctan2(py, px)
        <= np.pi,  # 120 to 180 degrees
        lambda px, py: -np.pi
        <= np.arctan2(py, px)
        <= -2 * np.pi / 3,  # -180 to -120 degrees
    ]
    ...
    for i, (mask_func, (start_angle, end_angle)) in enumerate(
        zip(segment_masks, angle_ranges)
    ):
        ...
        ax.plot(seg_x, seg_y, color=colors[i], linewidth=3, label=f"Segment {i + 1}")
