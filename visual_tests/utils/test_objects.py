...
class CurveFactory:
    ...
    @staticmethod
    def create_circle(
        center: Tuple[float, float] = (0, 0), radius: float = 2
    ) -> ConicSection:
        """
    ...
    @staticmethod
    def create_ellipse(
        center: Tuple[float, float] = (0, 0), a: float = 2, b: float = 1
    ) -> ConicSection:
        """
    ...
    def create_hyperbola(
        center: Tuple[float, float] = (0, 0), a: float = 1, b: float = 1
    ) -> ConicSection:
        """
    ...
    @staticmethod
    def create_parabola(
        vertex: Tuple[float, float] = (0, 0), direction: str = "up", scale: float = 1
    ) -> ConicSection:
        """
    ...
    def create_trimmed_circle(
        ...
        if mask_function is None:
            ...
            def mask_function(px, py):
                return py >= center[1]

        return TrimmedImplicitCurve(circle, mask_function)

    @staticmethod
    def create_composite_circle_quarters(
        center: Tuple[float, float] = (0, 0), radius: float = 1.5
    ) -> CompositeCurve:
        """
...
class RegionFactory:
    ...
    def create_circle_region(
        ...
        try:
            ...
            # Fallback: build a closed composite from
            # four quarter-circle segments
            cx, cy = center

            def q1_mask(px, py):
                return (px >= cx) and (py >= cy)  # NE

            def q2_mask(px, py):
                return (px <= cx) and (py >= cy)  # NW

            def q3_mask(px, py):
                return (px <= cx) and (py <= cy)  # SW

            def q4_mask(px, py):
                return (px >= cx) and (py <= cy)  # SE
    ...
    @staticmethod
    def create_triangle_region(
        vertices: List[Tuple[float, float]],
    ) -> AreaRegion:
        """
        Create a triangular area region from three vertices
        ...
        # using the working original method
        segments = []
        ...
        edge1_segment = TrimmedImplicitCurve(
            edge1_curve, edge1_mask, endpoints=[v1, v2]
        )
        ...
        edge2_segment = TrimmedImplicitCurve(
            edge2_curve, edge2_mask, endpoints=[v2, v3]
        )
        ...
        edge3_segment = TrimmedImplicitCurve(
            edge3_curve, edge3_mask, endpoints=[v3, v1]
        )
        ...
        # Annotate as convex polygon for fast, correct
        # region containment. For CCW-ordered vertices,
        # a point is inside if it's on the left side of
        # each directed edge.
        #
        # For edge (x1,y1)->(x2,y2), half-space:
        #   (x2-x1)*(y-y1) - (y2-y1)*(x-x1) >= 0
        # Rearranging:
        #   -(y2-y1)*x + (x2-x1)*y
        #   + (y2-y1)*x1 - (x2-x1)*y1 >= 0
        # CompositeCurve expects Ax+By+C <= tol,
        # so we negate:
        #   (y2-y1)*x - (x2-x1)*y
        #   - (y2-y1)*x1 + (x2-x1)*y1 <= 0
        edges_abc = []
    ...
    def create_rectangle_region(
        ...
        for i in range(4):
            ...
            trimmed_edge = TrimmedImplicitCurve(
                line_curve, edge_mask, endpoints=[v1, v2]
            )
        ...
        # Tag as axis-aligned square/rectangle for
        # fast path in CompositeCurve.contains/evaluate
    ...
    @staticmethod
    def create_region_with_hole(
        outer_region: AreaRegion, hole_region: AreaRegion
    ) -> AreaRegion:
        """
