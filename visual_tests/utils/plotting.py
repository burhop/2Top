...
class PlotManager:
                    ...
                    for j in range(X.shape[1]):
                        ...
                        try:
                            if hasattr(region, "contains_boundary"):
                                boundary_mask[i, j] = region.contains_boundary(
                                    X[i, j], Y[i, j]
                                )
                            else:
                                boundary_mask[i, j] = region.contains(
                                    X[i, j], Y[i, j], region_containment=False
                                )
                        except Exception:
    ...
    def save_or_show(
        self, filename: Optional[str] = None, dpi: int = 100, name: Optional[str] = None
    ) -> None:
        try:
            plt.tight_layout()
    ...
    def print_statistics(
        ...
        print(
            f"Inside points:     {inside_count} ({100 * inside_count / total_points:.1f}%)"
        )

        if boundary_mask is not None:
            boundary_count = np.sum(boundary_mask)
            outside_count = (
                total_points - inside_count - boundary_count
            )
            bnd_pct = 100 * boundary_count / total_points
            print(
                f"Boundary points:   {boundary_count}"
                f" ({bnd_pct:.1f}%)"
            )
            out_pct = 100 * outside_count / total_points
            print(
                f"Outside points:    {outside_count}"
                f" ({out_pct:.1f}%)"
            )
        else:
