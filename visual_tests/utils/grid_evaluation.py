...
class GridEvaluator:
    ...
    def create_grid(
        self, bounds: Tuple[float, float, float, float] = None, grid_size: int = None
    ...
    def evaluate_curve_over_grid(
        self, curve: Any, X: np.ndarray, Y: np.ndarray, handle_errors: bool = True
    ) -> np.ndarray:
        """
        ...
        if error_count > 0 and handle_errors:
            total_points = X.size
            print(
                f"Warning: {error_count}/{total_points} points failed evaluation "
                f"({100 * error_count / total_points:.1f}%)"
            )

        return Z

    def evaluate_region_containment(
        self,
        region: Any,
        X: np.ndarray,
        Y: np.ndarray,
        test_boundary: bool = False,
        ...
        for chunk_idx in range(num_chunks):
            ...
            if num_chunks > 1:
                ...
                print(
                    f"    Progress: {progress:.1f}% ({end_idx}/{total_points} points)"
                )
        ...
        if test_boundary:
            ...
            boundary_curve = getattr(region, "outer_boundary", None) or getattr(
                ...
                try:
                    ...
                    if curves:
                        ...
                        for i in range(total_points):
                            try:
                                boundary_flat[i] = region.contains(
                                    X_flat[i], Y_flat[i], region_containment=False
                                )
                            except Exception:
        ...
        if error_count > 0 and handle_errors:
            print(
                f"Warning: {error_count}/{total_points} points failed containment test "
                f"({100 * error_count / total_points:.1f}%)"
            )
    ...
    def print_test_results(self, results: Dict[str, Any]) -> None:
        ...
        for result in results["points"]:
            ...
            if "value" in result:  # Curve test
                print(
                    f"  Point {point_str} {desc_str}: {result['status']} (value: {result['value']:.6f})"
                )
            else:  # Region test
    ...
    def analyze_grid_statistics(
        self, inside_mask: np.ndarray, boundary_mask: Optional[np.ndarray] = None
    ) -> Dict[str, Any]:
        """
    ...
    def print_grid_statistics(self, stats: Dict[str, Any]) -> None:
        ...
        print(
            f"Inside points:     {stats['inside_count']} ({stats['inside_percentage']:.1f}%)"
        )

        if "boundary_count" in stats:
            print(
                f"Boundary points:   {stats['boundary_count']} ({stats['boundary_percentage']:.1f}%)"
            )
            print(
                f"Outside points:    {stats['outside_count']} ({stats['outside_percentage']:.1f}%)"
            )
        else:
            print(
                f"Outside points:    {stats['outside_count']} ({stats['outside_percentage']:.1f}%)"
    ...
    def create_focused_grid(
        self, center: Tuple[float, float], size: float, grid_size: int = None
    ) -> Tuple[np.ndarray, np.ndarray]:
        """
