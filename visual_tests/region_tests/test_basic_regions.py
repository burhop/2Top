...
def test_basic_area_regions():
    ...
    # Set up the plot
    fig, axes = plot_manager.setup_figure(
        1, 3, figsize=(15, 5), suptitle="Basic Area Regions"
    )
    ...
    regions = [
        ...
        (
            "Triangle",
            RegionFactory.create_triangle_region([(-2, -1.5), (2, -1.5), (0, 2)]),
        ),
    ...
    for i, (name, region) in enumerate(regions):
        ...
        plot_manager.plot_region_filled(
            ax,
            region,
            X,
            Y,
...
def test_region_containment():
    ...
    # Test each region
    regions = [("Circle (radius 2)", circle_region), ("Triangle", triangle_region)]
...
def test_region_statistics():
    ...
    regions = [
        ...
        (
            "Large Rectangle",
            RegionFactory.create_rectangle_region((-2.5, -1.5), (2.5, 1.5)),
        ),
    ]
    ...
    for i, (name, region) in enumerate(regions):
        ...
        if "Circle" in name:
            radius_str = (
                name.split()[-1] if name.split()[-1].replace(".", "").isdigit() else "2"
            )
            try:
                radius = float(radius_str)
                theoretical_area = np.pi * radius**2
                grid_area = (
                    (6**2) * stats["inside_count"] / stats["total_points"]
                )  # Grid covers 6x6 area
...
def test_region_boundaries():
    ...
    # Set up the plot
    fig, axes = plot_manager.setup_figure(
        2, 2, figsize=(12, 12), suptitle="Region Boundary Detection"
    )
    ...
    regions_and_grids = [
        (
            "Circle",
            RegionFactory.create_circle_region((0, 0), 1.5),
            grid_evaluator.create_focused_grid((0, 0), 2.5, 80),
        ),
        (
            "Triangle",
            RegionFactory.create_triangle_region([(-1.5, -1), (1.5, -1), (0, 1.5)]),
            grid_evaluator.create_focused_grid((0, 0), 2.5, 80),
        ),
        (
            "Rectangle",
            RegionFactory.create_rectangle_region((-1, -0.8), (1, 0.8)),
            grid_evaluator.create_focused_grid((0, 0), 2, 80),
        ),
        (
            "Small Circle",
            RegionFactory.create_circle_region((0, 0), 0.8),
    ...
    for i, (name, region, (X, Y)) in enumerate(regions_and_grids):
        ...
        total_points = X.size
        print(
            f"  Boundary points: {boundary_count}/{total_points} ({100 * boundary_count / total_points:.2f}%)"
        )
...
def test_region_comparison():
    ...
    # Set up the plot
    fig, axes = plot_manager.setup_figure(
        2, 3, figsize=(15, 10), suptitle="Region Comparison"
    )
    ...
    print(f"{'=' * 80}")
    print(
        f"{'Region':<20} {'Inside%':<10} {'Boundary%':<12} {'Outside%':<10} {'Area Est.':<10}"
    )
    print(f"{'-' * 80}")
    ...
    for name, stats in region_stats:
        ...
        print(
            f"{name:<20} {stats['inside_percentage']:<10.1f} "
