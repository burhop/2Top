...
class ConicSection(ImplicitCurve):
    ...
    def _extract_coefficients(self) -> dict:
        ...
        try:
            ...
            try:
                poly = sp.Poly(expanded_expr, x, y)
                coeffs_dict = poly.as_dict()
                A = float(coeffs_dict.get((2, 0), 0.0))
                B = float(coeffs_dict.get((1, 1), 0.0))
                C = float(coeffs_dict.get((0, 2), 0.0))
                D = float(coeffs_dict.get((1, 0), 0.0))
                E = float(coeffs_dict.get((0, 1), 0.0))
                F = float(coeffs_dict.get((0, 0), 0.0))
            except Exception:
                # Fallback to symbolic coeff extraction with variable isolation
                ...
                D = expanded_expr.subs(y, 0).coeff(x, 1)  # Pure x coefficient
                E = expanded_expr.subs(x, 0).coeff(y, 1)  # Pure y coefficient
        ...
        self._coefficients = {"A": A, "B": B, "C": C, "D": D, "E": E, "F": F}
    ...
    def conic_type(self) -> str:
        ...
        # Handle degenerate/linear cases first
        if abs(A) < 1e-12 and abs(B) < 1e-12 and abs(C) < 1e-12:
            D, E = coeffs["D"], coeffs["E"]
    ...
    def bounding_box(self) -> Tuple[float, float, float, float]:
        ...
        if conic_type == "circle":
            ...
            if (
                abs(A) < 1e-12
            ):  # Degenerate case (should not reach here due to assertion)
            ...
            assert abs(A) > 1e-12 and abs(C) > 1e-12, (
                f"Ellipse must have A ≠ 0 and C ≠ 0, but A={A}, C={C}"
            )
            ...
            if abs(B) < 1e-12:  # Axis-aligned ellipse
                ...
                constant = (
                    A * center_x**2 + B * center_x * center_y + C * center_y**2 - F
                )
        ...
        elif conic_type in ("degenerate", "line"):
    ...
    def from_dict(cls, data: dict) -> "ConicSection":
        ...
        if data.get("type") not in ["ConicSection", "ImplicitCurve"]:
            raise ValueError(
                f"Invalid type: expected 'ConicSection' or 'ImplicitCurve', got {data.get('type')}"
            )
