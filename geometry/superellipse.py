...
class Superellipse(ImplicitCurve):
    ...
    def bounding_box(self) -> tuple:
        """
        Return the exact analytical bounding box of the superellipse.

        For the curve |x/a|^n + |y/b|^n = 1, the bounding box is simply
        the rectangle [-a, a] x [-b, b] with a small padding for rendering.
        ...
        pad_x = self.a * 0.05
        pad_y = self.b * 0.05
        return (-self.a - pad_x, self.a + pad_x, -self.b - pad_y, self.b + pad_y)

    def get_center(self) -> tuple:
        """
        Return the centre of the superellipse.

        Currently always (0.0, 0.0). This method exists as an extension point
        for future translated superellipse support.
    ...
    def from_dict(cls, data: dict) -> "Superellipse":
        ...
        if data.get("type") != "Superellipse":
            raise ValueError(
                f"Invalid type: expected 'Superellipse', got {data.get('type')}"
            )
