...
def create_square_from_edges(
    ...
    segments = [
        ...
        TrimmedImplicitCurve(
            bottom_line, 
            lambda X, Y, ymin=ymin, xmin=xmin, xmax=xmax, e=eps: (X >= xmin - e) and (X <= xmax + e) and (abs(Y - ymin) <= e),
            endpoints=[(xmin, ymin), (xmax, ymin)]
        ),
        # Right edge: from (xmax, ymin) to (xmax, ymax)
        TrimmedImplicitCurve(
            right_line, 
            lambda X, Y, xmax=xmax, ymin=ymin, ymax=ymax, e=eps: (Y >= ymin - e) and (Y <= ymax + e) and (abs(X - xmax) <= e),
            endpoints=[(xmax, ymin), (xmax, ymax)]
        ),
        # Top edge: from (xmax, ymax) to (xmin, ymax)
        TrimmedImplicitCurve(
            top_line, 
            lambda X, Y, ymax=ymax, xmin=xmin, xmax=xmax, e=eps: (X >= xmin - e) and (X <= xmax + e) and (abs(Y - ymax) <= e),
            endpoints=[(xmax, ymax), (xmin, ymax)]
        ),
        # Left edge: from (xmin, ymax) to (xmin, ymin)
        TrimmedImplicitCurve(
            left_line, 
            lambda X, Y, xmin=xmin, ymin=ymin, ymax=ymax, e=eps: (Y >= ymin - e) and (Y <= ymax + e) and (abs(X - xmin) <= e),
...
def create_L_shape(variables: Optional[Tuple[sp.Symbol, sp.Symbol]] = None) -> CompositeCurve:
    """
    ...
    vertical_mask = lambda x_val, y_val: (-0.65 <= x_val <= -0.35) and (-1.05 <= y_val <= 0.05)  # Increased tolerance
    ...
    horizontal_mask = lambda x_val, y_val: (-0.55 <= x_val <= 0.55) and (-1.15 <= y_val <= -0.85)  # Increased tolerance
...
def create_T_shape(variables: Optional[Tuple[sp.Symbol, sp.Symbol]] = None) -> CompositeCurve:
    """
    ...
    horizontal_left_mask = lambda x_val, y_val: (-1.05 <= x_val <= 0.05) and (0.35 <= y_val <= 0.65)  # Increased tolerance
    segments.append(TrimmedImplicitCurve(horizontal_left, horizontal_left_mask, endpoints=[(-1, 0.5), (0, 0.5)]))
    
    # Vertical stem: x = 0, y from 0.5 to -1
    vertical = PolynomialCurve(x, variables)
    vertical_mask = lambda x_val, y_val: (-0.15 <= x_val <= 0.15) and (-1.05 <= y_val <= 0.55)  # Increased tolerance
    segments.append(TrimmedImplicitCurve(vertical, vertical_mask, endpoints=[(0, 0.5), (0, -1)]))
    
    # Horizontal top: y = 0.5, x from 0 to 1 (right part)
    horizontal_right = PolynomialCurve(y - 0.5, variables)
    horizontal_right_mask = lambda x_val, y_val: (-0.05 <= x_val <= 1.05) and (0.35 <= y_val <= 0.65)  # Increased tolerance
    segments.append(TrimmedImplicitCurve(horizontal_right, horizontal_right_mask, endpoints=[(0, 0.5), (1, 0.5)]))
    
    return CompositeCurve(segments, validate_continuity=True)


def create_triangle(variables: Optional[Tuple[sp.Symbol, sp.Symbol]] = None) -> CompositeCurve:
    """
    ...
    bottom_mask = lambda x_val, y_val: (-1.05 <= x_val <= 1.05) and (-0.65 <= y_val <= -0.35)  # Increased tolerance
    bottom_segment = TrimmedImplicitCurve(bottom, bottom_mask, endpoints=[(-1, -0.5), (1, -0.5)])
    
    # Right edge: from (1, -0.5) to (0, 1)
    # slope = (1 - (-0.5)) / (0 - 1) = -1.5
    # y - (-0.5) = -1.5(x - 1) => y = -1.5x + 1
    right = PolynomialCurve(y + 1.5*x - 1, variables)
    right_mask = lambda x_val, y_val: (-0.05 <= x_val <= 1.05) and (-0.55 <= y_val <= 1.05)  # Increased tolerance
    right_segment = TrimmedImplicitCurve(right, right_mask, endpoints=[(1, -0.5), (0, 1)])
    
    # Left edge: from (0, 1) to (-1, -0.5)
    # slope = (-0.5 - 1) / (-1 - 0) = 1.5
    # y - 1 = 1.5(x - 0) => y = 1.5x + 1
    left = PolynomialCurve(y - 1.5*x - 1, variables)
    left_mask = lambda x_val, y_val: (-1.05 <= x_val <= 0.05) and (-0.55 <= y_val <= 1.05)  # Increased tolerance
    left_segment = TrimmedImplicitCurve(left, left_mask, endpoints=[(0, 1), (-1, -0.5)])
    
    return CompositeCurve([bottom_segment, right_segment, left_segment], validate_continuity=True)


def create_house_shape(variables: Optional[Tuple[sp.Symbol, sp.Symbol]] = None) -> CompositeCurve:
    """
    ...
    bottom_mask = lambda x_val, y_val: (-0.55 <= x_val <= 0.55) and (-1.15 <= y_val <= -0.85)  # Increased tolerance
    ...
    right_mask = lambda x_val, y_val: (0.35 <= x_val <= 0.65) and (-1.05 <= y_val <= 0.05)  # Increased tolerance
    ...
    roof_right_mask = lambda x_val, y_val: (-0.05 <= x_val <= 0.55) and (-0.05 <= y_val <= 0.55)  # Increased tolerance
    ...
    roof_left_mask = lambda x_val, y_val: (-0.55 <= x_val <= 0.05) and (-0.05 <= y_val <= 0.55)  # Increased tolerance
    ...
    left_mask = lambda x_val, y_val: (-0.65 <= x_val <= -0.35) and (-1.05 <= y_val <= 0.05)  # Increased tolerance
    segments.append(TrimmedImplicitCurve(left, left_mask, endpoints=[(-0.5, 0), (-0.5, -1)]))
    
    return CompositeCurve(segments, validate_continuity=True)


def create_zigzag_pattern(variables: Optional[Tuple[sp.Symbol, sp.Symbol]] = None) -> CompositeCurve:
    """
    ...
    seg1_mask = lambda x_val, y_val: (-1.05 <= x_val <= 0.05) and (-0.55 <= y_val <= 0.55)  # Increased tolerance
    segments.append(TrimmedImplicitCurve(seg1, seg1_mask, endpoints=[(-1, -0.5), (0, 0.5)]))
    
    # Segment 2: from (0, 0.5) to (1, -0.5) - diagonal down
    # Line equation: y - 0.5 = -1(x - 0) => y = -x + 0.5
    seg2 = PolynomialCurve(y + x - 0.5, variables)
    seg2_mask = lambda x_val, y_val: (-0.05 <= x_val <= 1.05) and (-0.55 <= y_val <= 0.55)  # Increased tolerance
    segments.append(TrimmedImplicitCurve(seg2, seg2_mask, endpoints=[(0, 0.5), (1, -0.5)]))
    
    return CompositeCurve(segments, validate_continuity=True)


def create_staircase(variables: Optional[Tuple[sp.Symbol, sp.Symbol]] = None) -> CompositeCurve:
    """
    ...
    h1_mask = lambda x_val, y_val: (-1.05 <= x_val <= -0.45) and (-1.15 <= y_val <= -0.85)  # Increased tolerance
    ...
    v1_mask = lambda x_val, y_val: (-0.65 <= x_val <= -0.35) and (-1.05 <= y_val <= -0.45)  # Increased tolerance
    segments.append(TrimmedImplicitCurve(v1, v1_mask, endpoints=[(-0.5, -1), (-0.5, -0.5)]))
    
    # Step 2: horizontal from (-0.5, -0.5) to (0, -0.5)
    h2 = PolynomialCurve(y + 0.5, variables)
    h2_mask = lambda x_val, y_val: (-0.55 <= x_val <= 0.05) and (-0.65 <= y_val <= -0.35)  # Increased tolerance
    segments.append(TrimmedImplicitCurve(h2, h2_mask, endpoints=[(-0.5, -0.5), (0, -0.5)]))
    
    # Step 2: vertical from (0, -0.5) to (0, 0)
    v2 = PolynomialCurve(x, variables)
    v2_mask = lambda x_val, y_val: (-0.15 <= x_val <= 0.15) and (-0.55 <= y_val <= 0.05)  # Increased tolerance
    ...
    h3_mask = lambda x_val, y_val: (-0.05 <= x_val <= 0.55) and (-0.15 <= y_val <= 0.15)  # Increased tolerance
    ...
    v3_mask = lambda x_val, y_val: (0.35 <= x_val <= 0.65) and (-0.05 <= y_val <= 0.55)  # Increased tolerance
    segments.append(TrimmedImplicitCurve(v3, v3_mask, endpoints=[(0.5, 0), (0.5, 0.5)]))
    
    return CompositeCurve(segments, validate_continuity=True)


def create_figure_eight(variables: Optional[Tuple[sp.Symbol, sp.Symbol]] = None) -> CompositeCurve:
    """
    ...
    upper_right = TrimmedImplicitCurve(
        upper_circle, 
        lambda x_val, y_val, r=radius, tol=tolerance: (x_val >= -tol) and (x_val**2 + (y_val - 0.5)**2 <= (r + tol)**2),
    ...
    return CompositeCurve(segments, validate_continuity=False)  # Disable strict validation due to complex geometry

# Mixed Implicit Curve Composite Factories

def create_circle_line_hybrid(center: Tuple[float, float] = (0, 0), 
                             radius: float = 1.0,
                             variables: Optional[Tuple[sp.Symbol, sp.Symbol]] = None) -> CompositeCurve:
    """
    ...
    semicircle = TrimmedImplicitCurve(
        circle,
        lambda x_val, y_val, cx=cx, cy=cy, r=radius: (x_val >= cx - 0.05) and ((x_val - cx)**2 + (y_val - cy)**2 <= (r + 0.1)**2),  # Increased tolerance
    ...
    line_segment = TrimmedImplicitCurve(
        line,
        lambda x_val, y_val, cx=cx, cy=cy, r=radius: (cx - 0.15 <= x_val <= cx + 0.15) and (cy - r - 0.05 <= y_val <= cy + r + 0.05),  # Increased tolerance
        endpoints=[(cx, cy + radius), (cx, cy - radius)]
    )
...
def create_ellipse_parabola_hybrid(variables: Optional[Tuple[sp.Symbol, sp.Symbol]] = None) -> CompositeCurve:
    """
    ...
    ellipse_lower = TrimmedImplicitCurve(
        ellipse,
        lambda x_val, y_val, iy=intersection_y: (y_val <= iy + 0.05) and (x_val**2/4 + y_val**2 <= 1.2),
...
def create_multi_conic_flower(petals: int = 6, 
                             variables: Optional[Tuple[sp.Symbol, sp.Symbol]] = None) -> CompositeCurve:
    """
    ...
    for i in range(petals):
        ...
        if curve_type == 0:
            # Circle petal
            curve = ConicSection((x - center_x)**2 + (y - center_y)**2 - 0.16, variables)
            mask = lambda x_val, y_val, cx=center_x, cy=center_y: (x_val - cx)**2 + (y_val - cy)**2 <= 0.17
            ...
            a, b = 0.5, 0.2
            curve_expr = (((x - center_x) * cos_a + (y - center_y) * sin_a)**2 / a**2 + 
                         ((-(x - center_x) * sin_a + (y - center_y) * cos_a)**2 / b**2) - 1)
            curve = ConicSection(curve_expr, variables)
            mask = lambda x_val, y_val, cx=center_x, cy=center_y: (x_val - cx)**2 + (y_val - cy)**2 <= 0.3
            
        else:
            # Parabola petal (opening toward center)
            # Simplified: use a small circle for now (parabola rotation is complex)
            curve = ConicSection((x - center_x)**2 + (y - center_y)**2 - 0.09, variables)
            mask = lambda x_val, y_val, cx=center_x, cy=center_y: (x_val - cx)**2 + (y_val - cy)**2 <= 0.1
...
def create_superellipse_circle_hybrid(variables: Optional[Tuple[sp.Symbol, sp.Symbol]] = None) -> CompositeCurve:
    """
    ...
    superellipse_right = TrimmedImplicitCurve(
        superellipse,
        lambda x_val, y_val: (x_val >= -0.05) and (abs(x_val)**4 + abs(y_val)**4 <= 1.15),  # Increased tolerance
        endpoints=[(0, -1), (0, 1)]
    )
    ...
    circle_left = TrimmedImplicitCurve(
        circle,
        lambda x_val, y_val: (x_val <= 0.05) and ((x_val + 0.5)**2 + y_val**2 <= 1.4),  # Increased tolerance
        endpoints=[(0, 1), (0, -1)]
    )
...
def create_spiral_approximation(turns: int = 3, 
                               variables: Optional[Tuple[sp.Symbol, sp.Symbol]] = None) -> CompositeCurve:
    """
    ...
    for i in range(total_segments):
        ...
        center_x = (i * 0.1) * np.cos(angle_offset)
        center_y = (i * 0.1) * np.sin(angle_offset)
        
        # Quarter circle
        circle = ConicSection((x - center_x)**2 + (y - center_y)**2 - radius**2, variables)
        
        # Determine quadrant
        quadrant = i % 4
        if quadrant == 0:  # First quadrant
            mask = lambda x_val, y_val, cx=center_x, cy=center_y, r=radius: (x_val >= cx) and (y_val >= cy) and ((x_val - cx)**2 + (y_val - cy)**2 <= (r + 0.1)**2)
    ...
    # Spiral segments may not connect perfectly, so disable strict validation
    return CompositeCurve(segments, validate_continuity=False)


def create_heart_shape(variables: Optional[Tuple[sp.Symbol, sp.Symbol]] = None) -> CompositeCurve:
    """
    ...
    left_lobe = TrimmedImplicitCurve(
        left_circle,
        lambda x_val, y_val: (x_val >= -1.05) and (y_val >= -0.05) and ((x_val + 0.5)**2 + (y_val - 0.25)**2 <= 0.35),
    ...
    return CompositeCurve([left_lobe, right_lobe, heart_bottom], validate_continuity=False)
    
    return CompositeCurve([left_lobe, right_lobe, heart_bottom], validate_continuity=False)


def create_robust_square(corner1: Tuple[float, float] = (0.0, 0.0),
...
def create_lens_shape(variables: Optional[Tuple[sp.Symbol, sp.Symbol]] = None) -> CompositeCurve:
    """
    ...
    left_arc = TrimmedImplicitCurve(
        left_circle,
        lambda x_val, y_val: (x_val >= -0.05) and ((x_val + 0.5)**2 + y_val**2 <= 1.2),  # Right half with tolerance
    ...
    right_arc = TrimmedImplicitCurve(
        right_circle,
        lambda x_val, y_val: (x_val <= 0.05) and ((x_val - 0.5)**2 + y_val**2 <= 1.2),  # Left half with tolerance
        endpoints=[(0, -intersection_y), (0, intersection_y)]
    )
