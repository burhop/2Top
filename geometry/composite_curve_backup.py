...
class CompositeCurve(CompositePolygonMixin, ImplicitCurve):
    ...
    def _is_single_segment_closed(self) -> bool:
        ...
        if hasattr(segment, "base_curve") and isinstance(
            segment.base_curve, ConicSection
        ):
            ...
            if self.is_closed():
                if region_containment:
                    ...
                    if getattr(self, "_is_square", False):
                        ...
                        return (
                            (x >= xmin - tolerance)
                            and (x <= xmax + tolerance)
                            and (y >= ymin - tolerance)
                            and (y <= ymax + tolerance)
                        )
        ...
        if self.is_closed():
            if region_containment:
                ...
                if getattr(self, "_is_square", False):
                    ...
                    return (
                        (x_array >= xmin - tolerance)
                        & (x_array <= xmax + tolerance)
                        & (y_array >= ymin - tolerance)
                        & (y_array <= ymax + tolerance)
                    )
