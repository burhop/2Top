...
class TestMixedImplicitComposites:
    ...
    def test_circle_line_composite(self):
        ...
        composite = CompositeCurve([circle_right, line_top, line_left, line_bottom], 
    ...
    def test_ellipse_parabola_composite(self):
        ...
        composite = CompositeCurve([ellipse_upper, parabola_lower], 
                                 validate_continuity=True)
    ...
    def test_superellipse_hyperbola_composite(self):
        ...
        hyperbola_shifted = ConicSection((self.x + 1)**2 - self.y**2 - 1, (self.x, self.y))
        ...
        composite = CompositeCurve([superellipse_right, hyperbola_right], 
    ...
    def test_multiple_conic_sections(self):
        ...
        composite = CompositeCurve([circle_q1, ellipse_q2, parabola_q3, circle_q4], 
                                 validate_continuity=True)
    ...
    def test_procedural_curve_composite(self):
        ...
        composite = CompositeCurve([sine_segment, line_return], 
                                 validate_continuity=True)
    ...
    def test_rfunction_composite(self):
        ...
        # R-function blend
        rfunction_curve = RFunctionCurve([circle1_expr, circle2_expr], 
                                       'union', (self.x, self.y))
        ...
        composite = CompositeCurve([rfunction_segment, line_top, line_left], 
                                 validate_continuity=False)
    ...
    def test_mixed_curve_performance(self):
        ...
        for i in range(5):
            ...
            if i % 3 == 0:
                # Circle
                curve = ConicSection((self.x - center_x)**2 + (self.y - center_y)**2 - 0.1, 
                                   (self.x, self.y))
            elif i % 3 == 1:
                # Ellipse
                curve = ConicSection((self.x - center_x)**2/0.2 + (self.y - center_y)**2/0.05 - 1, 
                                   (self.x, self.y))
            else:
                # Line
                curve = PolynomialCurve(self.y - center_y - 0.5*(self.x - center_x), 
                                      (self.x, self.y))
            
            segment = TrimmedImplicitCurve(
                curve,
                lambda x, y, cx=center_x, cy=center_y: (x - cx)**2 + (y - cy)**2 <= 0.2,
    ...
    def test_create_flower_shape(self):
        ...
        def create_flower_shape():
            ...
            for i in range(6):
                ...
                if i % 2 == 0:
                    # Circle petal
                    curve = ConicSection((self.x - center_x)**2 + (self.y - center_y)**2 - 0.16, 
                                       (self.x, self.y))
                else:
                    ...
                    curve_expr = (((self.x - center_x) * cos_a + (self.y - center_y) * sin_a)**2 / a**2 + 
                                ((-(self.x - center_x) * sin_a + (self.y - center_y) * cos_a)**2 / b**2) - 1)
                    curve = ConicSection(curve_expr, (self.x, self.y))
                
                # Create petal segment
                segment = TrimmedImplicitCurve(
                    curve,
                    lambda x, y, cx=center_x, cy=center_y: (x - cx)**2 + (y - cy)**2 <= 0.25,
    ...
    def test_create_spiral_approximation(self):
        ...
        def create_spiral_approximation():
            ...
            for i in range(4):
                ...
                # Quarter circle
                circle = ConicSection((self.x - center_offset)**2 + self.y**2 - radius**2, 
                                    (self.x, self.y))
                
                # Different quadrant for each segment
                if i % 4 == 0:
                    mask = lambda x, y, r=radius, cx=center_offset: x >= cx and y >= 0 and (x - cx)**2 + y**2 <= r**2 * 1.01
