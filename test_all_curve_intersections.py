...
def test_all_curve_intersections():
            ...
            try:
                ...
                if intersections:
                    ...
                    for k, (px, py) in enumerate(intersections):
                        ...
                        print(
                            f"      {k + 1}. ({px:7.4f}, {py:7.4f}) - error: {error:.2e}"
                        )
                else:
    ...
    print(
        f"  Average intersections per pair: {total_intersections / len(intersection_results):.1f}"
    )

    # Show pairs with most intersections
    sorted_results = sorted(
        intersection_results.items(), key=lambda x: len(x[1]), reverse=True
    )
    ...
    fig.suptitle(
        "Intersection Tests: Various Curve Combinations", fontsize=16, fontweight="bold"
    )
