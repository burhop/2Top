...
def test_comprehensive_composite_functionality():
    ...
    try:
        ...
        seg2 = TrimmedImplicitCurve(
            line2, lambda x, y: 0 <= y <= 1, endpoints=[(1, 0), (1, 1)]
        )
    ...
    try:
        ...
        seg2 = TrimmedImplicitCurve(
            line2, lambda x, y: 0 <= y <= 1, endpoints=[(2, 1), (2, 2)]
        )
    ...
    results = [
        (
            "Factory Functions",
            all_factory_passed,
            f"{sum(factory_results)}/{len(factory_results)}",
        ),
        ("Continuity Validation", continuity_passed, "2/2"),
        (
            "Core Methods",
            all_methods_passed,
            f"{sum(method_results)}/{len(method_results)}",
        ),
