...
def test_composite_shapes():
    ...
    try:
        ...
        vertical_mask = lambda x_val, y_val: (-0.1 <= x_val <= 0.1) and (
            -1 <= y_val <= 1
        )
        vertical_segment = TrimmedImplicitCurve(
            vertical, vertical_mask, endpoints=[(0, -1), (0, 1)]
        )

        # Horizontal line: y = 0, x from -1 to 1
        horizontal = PolynomialCurve(y, (x, y))
        horizontal_mask = lambda x_val, y_val: (-1 <= x_val <= 1) and (
            -0.1 <= y_val <= 0.1
        )
        horizontal_segment = TrimmedImplicitCurve(
            horizontal, horizontal_mask, endpoints=[(-1, 0), (1, 0)]
        )

        # Plus sign segments don't connect end-to-end, so disable continuity validation
        plus_sign = CompositeCurve(
            [vertical_segment, horizontal_segment], validate_continuity=False
        )
    ...
    print("\n📋 SUMMARY:")
    print(
        "Composite shapes with proper endpoints and continuity validation are working!"
    )
