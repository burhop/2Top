...
from geometry.curve_intersections import (
...
class ImprovedCurveVisualizerApp:
    ...
    def setup_control_panel(self, parent):
        ...
        title_label = ttk.Label(
            parent, text="Curve Visualizer", font=("Arial", 14, "bold")
        )
    ...
    def setup_add_curves(self, parent):
        ...
        # Custom expression
        ttk.Label(add_frame, text="Custom f(x,y) = 0:").pack(
        ...
        ttk.Button(add_frame, text="Add Custom", command=self.add_custom_curve).pack(
            pady=5
        )
    ...
    def setup_operations(self, parent):
        ...
        ttk.Button(
            ...
            command=self.find_all_intersections,
        ).pack(fill=tk.X, padx=5, pady=2)

        # R-function operations (clearly labeled)
        ttk.Label(
            ops_frame, text="R-Function Regions:", font=("Arial", 9, "bold")
        ...
        ttk.Button(
            region_frame, text="Union Region", command=self.add_union_region
        ).pack(side=tk.LEFT, padx=2)
        ttk.Button(
            region_frame, text="Intersect Region", command=self.add_intersect_region
        ).pack(side=tk.LEFT, padx=2)

        ttk.Label(
            ops_frame,
            text="(Creates new curves representing combined regions)",
            font=("Arial", 8),
            foreground="gray",
    ...
    def setup_plot_style(self):
        ...
        self.ax.set_title(
            "Implicit Curves with Intersection Points", fontsize=14, fontweight="bold"
        )
    ...
    def find_all_intersections(self):
        ...
        if len(self.curves) < 2:
            messagebox.showwarning(
                "Warning", "Need at least 2 curves to find intersections"
            )
            return
        ...
        for i in range(len(self.curves)):
            for j in range(i + 1, len(self.curves)):
                ...
                intersections = find_curve_intersections(
                    curve1, curve2, search_range=self.plot_range.get()
                )
        ...
        if self.intersection_points:
            result_msg = (
                f"Found {len(self.intersection_points)} intersection points:\n\n"
            )
    ...
    def add_intersect_region(self):
        ...
        if len(self.curves) < 2:
            messagebox.showwarning(
                "Warning", "Need at least 2 curves for intersection region"
            )
            return
