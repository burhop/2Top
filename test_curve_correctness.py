...
def test_curve_correctness():
    ...
    for (px, py), expected, desc in test_points:
        ...
        print(
            f"  {status} Circle at ({px},{py}): {result} (expected {expected}) - {desc}"
        )
    ...
    for (px, py), expected, desc in ellipse_tests:
        ...
        status = "✅" if abs(result - expected) < 1e-10 else "❌"
        print(
            f"  {status} Ellipse at ({px},{py}): {result} (expected {expected}) - {desc}"
        )
    ...
    for (px, py), expected, desc in line_tests:
        ...
        status = "✅" if abs(result - expected) < 1e-10 else "❌"
        print(
            f"  {status} Line at ({px},{py}): {result} (expected {expected}) - {desc}"
        )
    ...
    for (px, py), expected, desc in parabola_tests:
        ...
        status = "✅" if abs(result - expected) < 1e-10 else "❌"
        print(
            f"  {status} Parabola at ({px},{py}): {result} (expected {expected}) - {desc}"
        )
    ...
    for (px, py), expected, desc in super_tests:
        ...
        status = "✅" if abs(result - expected) < 1e-10 else "❌"
        print(
            f"  {status} Superellipse at ({px},{py}): {result} (expected {expected}) - {desc}"
        )
    ...
    for (px, py), expected, desc in square_tests:
        ...
        if expected is None:
            ...
            expected = expected_calc
        status = (
            "✅" if abs(result - expected) < 1e-6 else "❌"
        )  # Looser tolerance for high powers
        print(
            f"  {status} Super-square at ({px},{py}): {result:.6f} (expected {expected:.6f}) - {desc}"
        )
    ...
    for (px, py), expected, desc in proc_tests:
        ...
        status = "✅" if abs(result - expected) < 1e-10 else "❌"
        print(
            f"  {status} Procedural at ({px},{py}): {result} (expected {expected}) - {desc}"
        )
    ...
    for (px, py), expected, desc in union_tests:
        ...
        status = "✅" if abs(result - expected) < 1e-10 else "❌"
        print(
            f"  {status} Union at ({px},{py}): {result} (expected {expected}) - {desc}"
        )
    ...
    print(
        f"  {status} Vectorized circle: {circle_results} (expected {expected_results})"
    )
