...
def create_comprehensive_square_test():
    ...
    for square, name, color in squares:
        contour_sets = square.plot(ax=ax1, x_range=(-3, 3), y_range=(-3, 3), 
    ...
    for i, (square, name, color) in enumerate(squares):
        ...
        contour_sets = square.plot(ax=ax, x_range=x_range, y_range=y_range, 
        ...
        # Test containment
        inside = square.contains(center_x, center_y, tolerance=0.1, region_containment=True)
        boundary = square.contains(center_x, center_y, tolerance=0.1, region_containment=False)
    ...
    contourf = ax6.contourf(X, Y, Z, levels=[-2, 0, 2], colors=['lightblue', 'white'], alpha=0.7)
    ...
    # Summary
    print(f"\n📊 SUMMARY:")
    print(f"✅ All squares created successfully")
    print(f"✅ All squares plot with multiple contour sets")
    print(f"✅ All squares have correct containment logic")
    print(f"✅ Manual evaluation shows proper inside/outside regions")
    print(f"✅ Squares are working correctly!")
