...
class AdvancedCurveVisualizerApp:
    ...
    def setup_control_panel(self, parent):
        ...
        title_label = ttk.Label(
            scrollable_frame, text="Curve Controls", font=("Arial", 14, "bold")
        )
    ...
    def setup_curve_list(self, parent):
        ...
        list_scrollbar = ttk.Scrollbar(
            list_container, orient=tk.VERTICAL, command=self.curve_listbox.yview
        )
        ...
        ttk.Button(button_frame, text="Remove", command=self.remove_curve).pack(
            side=tk.LEFT, padx=2
        )
        ttk.Button(button_frame, text="Clear All", command=self.clear_curves).pack(
            side=tk.LEFT, padx=2
        )
        ttk.Button(button_frame, text="Duplicate", command=self.duplicate_curve).pack(
            side=tk.LEFT, padx=2
        )
    ...
    def setup_add_curve(self, parent):
        ...
        ttk.Button(ops_frame, text="Union", command=self.add_union).pack(
            side=tk.LEFT, padx=2, pady=2
        )
        ttk.Button(ops_frame, text="Intersect", command=self.add_intersection).pack(
            side=tk.LEFT, padx=2, pady=2
        )
        ttk.Button(ops_frame, text="Difference", command=self.add_difference).pack(
            side=tk.LEFT, padx=2, pady=2
        )
        ttk.Button(ops_frame, text="Blend", command=self.add_blend).pack(
            side=tk.LEFT, padx=2, pady=2
        )
    ...
    def setup_custom_expression(self, parent):
        ...
        ttk.Label(expr_frame, text="Enter f(x,y) = 0:").pack(
        ...
        ttk.Button(
            expr_frame, text="Add Custom Curve", command=self.add_custom_curve
        ).pack(pady=5)
        ...
        for example in examples:
            label = ttk.Label(
                examples_frame, text=example, font=("Arial", 7), foreground="gray"
            )
            label.pack(anchor=tk.W)
            label.bind(
                "<Button-1>",
                lambda e, ex=example.split("  ")[0]: self.custom_expr.set(ex),
            )
    ...
    def setup_plot_settings(self, parent):
        ...
        # Checkboxes
        ttk.Checkbutton(settings_frame, text="Show Grid", variable=self.show_grid).pack(
            anchor=tk.W, padx=5, pady=2
        )
        ttk.Checkbutton(
            settings_frame, text="Show Filled Regions", variable=self.show_filled
        ).pack(anchor=tk.W, padx=5, pady=2)
        ttk.Checkbutton(
            settings_frame, text="Show Gradient Vectors", variable=self.show_gradients
        ).pack(anchor=tk.W, padx=5, pady=2)
    ...
    def setup_file_operations(self, parent):
        ...
        ttk.Button(file_frame, text="Save Plot", command=self.save_plot).pack(
            side=tk.LEFT, padx=2, pady=2
        )
    ...
    def setup_gallery_tab(self):
        ...
        ttk.Label(gallery_frame, text="Curve Gallery", font=("Arial", 16, "bold")).pack(
            pady=(0, 10)
        )
        ...
        for name, expr, desc in gallery_items:
            ...
            ttk.Label(item_frame, text=f"Expression: {expr}", font=("Courier", 9)).pack(
                anchor=tk.W, padx=5, pady=2
            )
            ttk.Label(item_frame, text=desc, font=("Arial", 9), foreground="gray").pack(
                anchor=tk.W, padx=5, pady=2
            )
            ...
            ttk.Button(
                item_frame,
                text="Add to Plot",
                command=lambda e=expr, n=name: self.add_gallery_curve(e, n),
            ).pack(pady=5)
    ...
    def setup_plot_style(self):
        ...
        self.ax.set_title(
            "Implicit Curves Visualization", fontsize=14, fontweight="bold"
        )
    ...
    def add_intersection(self):
        ...
        if len(self.curves) >= 2:
            ...
            name = "Intersection of last 2 curves"
            self.curves.append((name, curve))
            self.update_curve_list()
            self.update_plot()
        else:
    ...
    def add_difference(self):
        ...
        if len(self.curves) >= 2:
            ...
            name = "Difference of last 2 curves"
            self.curves.append((name, curve))
            self.update_curve_list()
            self.update_plot()
        else:
    ...
    def update_plot(self):
        ...
        for i, (name, curve) in enumerate(self.curves):
            try:
                ...
                if (
                    self.show_gradients.get() and i == 0
                ):  # Only for first curve to avoid clutter
