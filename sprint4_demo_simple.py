...
def demo_basic_operations():
    ...
    print("\nEvaluation results:")
    print(
        f"{'Point':<15} {'Union':<8} {'Intersect':<10} {'Difference':<10} {'Blend':<8}"
    )
    print("-" * 60)
    ...
    for x_val, y_val, desc in test_points:
        ...
        print(
            f"({x_val:4.1f}, {y_val:4.1f}){'':<5} {union_val:7.3f}  {intersect_val:9.3f}  {diff_val:9.3f}  {blend_val:7.3f}  {desc}"
        )
...
def demo_mixed_curve_types():
    ...
    print(
        f"  Blend(Polynomial, Superellipse, α=0.3): {mixed_blend.evaluate(0.5, 0.5):.6f}"
    )
...
def demo_serialization():
    ...
    for x_val, y_val in test_points:
        ...
        print(
            f"  ({x_val:4.1f}, {y_val:4.1f}): original={orig_val:.6f}, restored={rest_val:.6f}, diff={diff:.2e}"
        )
