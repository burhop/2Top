...
def test_composite_continuity():
    ...
    try:
        ...
        left_segment = TrimmedImplicitCurve(
            left_line, left_mask, endpoints=[(-1, -1), (0, 1)]
        )
        ...
        right_segment = TrimmedImplicitCurve(
            right_line, right_mask, endpoints=[(0, 1), (1, -1)]
        )
        ...
        bottom_segment = TrimmedImplicitCurve(
            bottom_line, bottom_mask, endpoints=[(1, -1), (-1, -1)]
        )
    ...
    try:
        ...
        gap_distance = np.sqrt(
            (seg2_start[0] - seg1_end[0]) ** 2 + (seg2_start[1] - seg1_end[1]) ** 2
        )
        print(
            f"  Gap between segments: {gap_distance:.3f} (should be ~0 for continuity)"
        )
        ...
        try:
            ...
            for i in range(len(shape.segments)):
                ...
                if curr_endpoints and next_endpoints:
                    ...
                    gaps.append(min_gap)
                    print(
                        f"    Gap {i}->{(i + 1) % len(shape.segments)}: {min_gap:.6f}"
                    )
