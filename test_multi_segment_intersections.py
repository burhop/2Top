...
def test_multi_segment_intersections():
    ...
    print("  Trimmed Circle: Right half of x² + y² = 1")
    print("  Line: y = 0")

    intersections = find_curve_intersections(
        trimmed_circle, line, search_range=2.0, grid_resolution=400, tolerance=1e-8
    )
    ...
    for i, (px, py) in enumerate(intersections):
        ...
        print(
            f"    {i + 1}. ({px:7.4f}, {py:7.4f}) - error: {error:.2e}, on_trimmed: {on_trimmed}"
        )
    ...
    intersections = find_curve_intersections(
        square, diagonal, search_range=2.0, grid_resolution=400, tolerance=1e-8
    )
    ...
    print("  Upper Circle: Upper half of x² + y² = 1")
    print("  Right Ellipse: Right half of x²/4 + y² = 1")
    ...
    print("  Circle Quarters: Composite circle with radius 1.5")
    print("  Line: x = 0.5")

    intersections = find_curve_intersections(
        circle_quarters, vertical, search_range=2.0, grid_resolution=400, tolerance=1e-8
    )
    ...
    for i, (px, py) in enumerate(intersections):
        ...
        print(
            f"    {i + 1}. ({px:7.4f}, {py:7.4f}) - error: {error:.2e}, on_quarters: {on_quarters}"
        )
