    ...
    try:
        ...
        for i, segment in enumerate(heart.segments):
            ...
            if hasattr(segment, "endpoints") and segment.endpoints:
                ...
                if abs(start_val) < 0.1 and abs(end_val) < 0.1:
                    print("  ✅ Endpoints are on curve")
                else:
                    print("  ❌ Endpoints not on curve!")
        ...
        for i in range(len(heart.segments) - 1):
            ...
            if seg1.endpoints and seg2.endpoints:
                ...
                distance = np.sqrt(
                    (end1[0] - start2[0]) ** 2 + (end1[1] - start2[1]) ** 2
                )
                ...
                if distance < 1e-6:
                    ...
                    print(
                        f"  Seg {i}→{i + 1}: ❌ Gap! {end1} → {start2} (distance: {distance:.6f})"
                    )
        ...
        if len(heart.segments) >= 2:
            ...
            if closure_distance < 1e-6:
                ...
                print(
                    f"  Closure: ❌ Open! {last_end} → {first_start} (distance: {closure_distance:.6f})"
                )
