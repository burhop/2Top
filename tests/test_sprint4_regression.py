        ...
        for curve in self.all_curves:
            for x, y in test_points:
                try:
                    ...
                    pytest.fail(
                        f"evaluate failed for {type(curve).__name__} at ({x}, {y}): {e}"
                    )
        ...
        for curve in self.all_curves:
            for x, y in test_points:
                try:
                    ...
                    pytest.fail(
                        f"gradient failed for {type(curve).__name__} at ({x}, {y}): {e}"
                    )
    ...
    def test_vectorized_evaluation_consistency(self):
        ...
        for curve in self.all_curves:
            try:
                ...
                pytest.fail(
                    f"Vectorized evaluate failed for {type(curve).__name__}: {e}"
                )
        ...
        for curve1, curve2 in self.curve_pairs:
            for alpha in alpha_values:
                ...
                for x, y in test_points:
                    ...
                    expected_val = (
                        curve1_val + curve2_val - np.sqrt(diff**2 + alpha**2)
                    ) / 2
...
class TestWrapperFunctionEquivalence:
    ...
    def test_intersect_wrapper_equivalence(self):
        ...
        direct_intersect = RFunctionCurve(
            self.curve1, self.curve2, operation="intersection"
        )
    ...
    def test_blend_wrapper_equivalence(self):
        ...
        direct_blend = RFunctionCurve(
            self.curve1, self.curve2, operation="blend", alpha=alpha
        )
...
class TestNestedConstructiveOperations:
    ...
    def setup_method(self):
        ...
        self.circle3 = ConicSection(
            (x - 0.5) ** 2 + (y - 0.5) ** 2 - 0.25, variables=(x, y)
        )
