...
class TestCompositeCurveConstructor:
    ...
    def test_constructor_with_various_segments(self):
        ...
        assert all(
            isinstance(seg, TrimmedImplicitCurve) for seg in composite_curve.segments
        )
...
class TestCompositeCurveIsClosedMethod:
    ...
    def test_is_closed_with_gaps(self):
        ...
        gapped_segments = [
            TrimmedImplicitCurve(
                circle, lambda x, y: x >= 0.1 and y >= 0.1
            ),  # First quadrant with gap
            TrimmedImplicitCurve(
                circle, lambda x, y: x <= -0.1 and y >= 0.1
            ),  # Second quadrant with gap
        ]
    ...
    def test_contains_with_partial_segments(self):
        ...
        test_cases = [
            ...
            (
                ...
                -1 / math.sqrt(2),
                False,
                "Point on third quadrant arc (not included)",
            ),
            (
                1 / math.sqrt(2),
                -1 / math.sqrt(2),
                False,
                "Point on fourth quadrant arc (not included)",
            ),
        ]
...
class TestCompositeCurveSpecializedMethods:
    ...
    def test_property_access(self):
        ...
        assert all(
            isinstance(seg, TrimmedImplicitCurve)
            for seg in self.composite_curve.segments
        )
...
class TestCompositeCurveConnectivity:
    ...
    def setup_method(self):
        ...
        self.connected_segments = [
            TrimmedImplicitCurve(
                self.line1, lambda x, y: x >= 0 and x <= 1
            ),  # Bottom edge
            TrimmedImplicitCurve(
                self.line2, lambda x, y: y >= 0 and y <= 1
            ),  # Right edge
            TrimmedImplicitCurve(
                self.line3, lambda x, y: x >= 0 and x <= 1
            ),  # Top edge
            TrimmedImplicitCurve(
                self.line4, lambda x, y: y >= 0 and y <= 1
            ),  # Left edge
        ]
        ...
        self.disconnected_segments = [
            TrimmedImplicitCurve(
                self.line1, lambda x, y: x >= 0 and x <= 0.5
            ),  # Partial bottom
            TrimmedImplicitCurve(
                self.line2, lambda x, y: y >= 0.5 and y <= 1
            ),  # Partial right
        ]
...
class TestCompositeCurveGoldenDigests:
    ...
    def test_overlapping_segments(self):
        ...
        assert (
            overlapping_composite.contains(1 / math.sqrt(2), 1 / math.sqrt(2)) == True
        )  # On both segments
...
class TestCompositeCurveExtensive:
    ...
    def test_fast_path_metadata_evaluation(self):
        ...
        np.testing.assert_array_equal(
            square.contains(xs, ys, region_containment=True), expected
        )
        ...
        np.testing.assert_array_equal(
            triangle.contains(
                np.array([0.5, 1.5]), np.array([0.5, 1.5]), region_containment=True
            ),
            np.array([True, False]),
        )
    ...
    def test_segment_modification_apis(self):
        ...
        with pytest.raises(
            ValueError, match="Cannot remove the last remaining segment"
        ):
