...
class TestAreaRegionConstructor:
    ...
    def test_constructor_validates_outer_boundary_closed(self):
        ...
        try:
            ...
            assert open_composite.is_closed(), (
                "Test setup error: boundary should be closed or raise error"
            )
    ...
    def test_constructor_validates_holes_closed(self):
        ...
        try:
            ...
            assert open_hole.is_closed(), (
                "Test setup error: hole should be closed or raise error"
            )
        except ValueError as e:
            assert "must be closed" in str(e)
...
class TestAreaRegionExtensive:
    ...
    def test_polygonal_area_calculation(self):
        ...
        def make_polygon_circle(radius, center=(0.0, 0.0), num_sides=64):
            ...
            vertices = [
                (cx + radius * np.cos(t), cy + radius * np.sin(t)) for t in theta
            ]
        ...
        hole_tri = create_polygon_from_edges(
    ...
    def test_field_strategy_integration(self):
        ...
        with pytest.raises(
            ValueError, match="does not represent a SignedDistanceStrategy"
        ):
        ...
        with pytest.raises(
            ValueError, match="does not represent a SignedDistanceField"
        ):
        ...
        with pytest.raises(
            ValueError, match="does not represent an OccupancyFillStrategy"
        ):
    ...
    def test_error_validations(self):
        ...
        # Strategy mismatch
        with pytest.raises(
            TypeError, match="strategy must be a FieldStrategy instance"
        ):
        ...
        seg1 = TrimmedImplicitCurve(
            line1, lambda x, y: -1 <= x <= 1, endpoints=((-1.0, 0.0), (1.0, 0.0))
        )
        ...
        with pytest.raises(
            ValueError,
            match="outer_boundary must have at least 3 segments to form a closed region",
        ):
        ...
        with pytest.raises(
            ValueError,
            match="holes\\[0\\] must have at least 3 segments to form a closed region",
        ):
