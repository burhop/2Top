...
class TestRFunctionCurveConstructor:
    ...
    def test_constructor_with_different_curve_types(self):
        ...
        combinations = [(conic, poly), (conic, superellipse), (poly, superellipse)]
...
class TestRFunctionCurveSharpOperations:
    ...
    def setup_method(self):
        ...
        self.circle1 = ConicSection(
            x**2 + y**2 - 1, variables=(x, y)
        )  # Centered at origin
        self.circle2 = ConicSection(
            (x - 1) ** 2 + y**2 - 1, variables=(x, y)
        )  # Centered at (1,0)

        # Union and intersection curves
        self.union_curve = RFunctionCurve(self.circle1, self.circle2, operation="union")
        self.intersection_curve = RFunctionCurve(
            self.circle1, self.circle2, operation="intersection"
        )
        self.difference_curve = RFunctionCurve(
            self.circle1, self.circle2, operation="difference"
        )
    ...
    def test_sharp_intersection_evaluation(self):
        ...
        for x, y, expected in test_cases:
            ...
            # Check inside/outside behavior
            if expected == "inside":
                assert intersection_val < 0, (
                    f"Point ({x}, {y}) should be inside intersection"
                )
            else:
                assert intersection_val > 0, (
                    f"Point ({x}, {y}) should be outside intersection"
                )
    ...
    def test_sharp_difference_evaluation(self):
        ...
        for x, y, expected in test_cases:
            ...
            # Check inside/outside behavior
            if expected == "inside":
                assert difference_val < 0, (
                    f"Point ({x}, {y}) should be inside difference"
                )
            else:
                assert difference_val > 0, (
                    f"Point ({x}, {y}) should be outside difference"
                )
    ...
    def test_vectorized_sharp_operations(self):
        ...
        np.testing.assert_allclose(
            intersection_results, expected_intersection, rtol=1e-10
        )
...
class TestRFunctionCurveSmoothBlending:
    ...
    def setup_method(self):
        ...
        self.blend_small = RFunctionCurve(
            self.circle1, self.circle2, operation="blend", alpha=0.1
        )
        self.blend_medium = RFunctionCurve(
            self.circle1, self.circle2, operation="blend", alpha=0.5
        )
        self.blend_large = RFunctionCurve(
            self.circle1, self.circle2, operation="blend", alpha=1.0
        )
    ...
    def test_smooth_blend_continuity(self):
        ...
        test_points = [(0.25, 0.0), (0.5, 0.0), (0.75, 0.0)]
    ...
    def test_smooth_blend_approaches_sharp_operations(self):
        ...
        tiny_alpha_blend = RFunctionCurve(
            circle1, circle2, operation="blend", alpha=1e-10
        )
...
class TestRFunctionCurveGradient:
    ...
    def setup_method(self):
        ...
        self.union_curve = RFunctionCurve(self.circle1, self.circle2, operation="union")
        self.blend_curve = RFunctionCurve(
            self.circle1, self.circle2, operation="blend", alpha=0.5
        )
...
class TestRFunctionCurveSerialization:
    ...
    def setup_method(self):
        ...
        self.intersection_curve = RFunctionCurve(
            self.circle, self.poly, operation="intersection"
        )
        self.difference_curve = RFunctionCurve(
            self.poly, self.superellipse, operation="difference"
        )
        self.blend_curve = RFunctionCurve(
            self.circle, self.ellipse, operation="blend", alpha=0.5
        )
...
class TestRFunctionCurveSpecializedMethods:
    ...
    def setup_method(self):
        ...
        self.union_curve = RFunctionCurve(circle1, circle2, operation="union")
        self.blend_curve = RFunctionCurve(
            circle1, circle2, operation="blend", alpha=0.5
        )
