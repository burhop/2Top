        ...
        for conic_curve, poly_curve in curve_pairs:
            for x, y in test_points:
                ...
                assert abs(conic_grad[0] - poly_grad[0]) < 1e-10, (
                    f"Gradient x mismatch at ({x}, {y})"
                )
                assert abs(conic_grad[1] - poly_grad[1]) < 1e-10, (
                    f"Gradient y mismatch at ({x}, {y})"
                )
        ...
        for conic_curve, poly_curve in curve_pairs:
            for x, y in test_points:
                try:
                    ...
                    assert abs(conic_normal[0] - poly_normal[0]) < 1e-10, (
                        f"Normal x mismatch at ({x}, {y})"
                    )
                    assert abs(conic_normal[1] - poly_normal[1]) < 1e-10, (
                        f"Normal y mismatch at ({x}, {y})"
                    )
                except ValueError:
