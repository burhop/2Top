...
class TestTrimmedImplicitCurveContains:
    ...
    def setup_method(self):
        ...
        self.first_quadrant = TrimmedImplicitCurve(
            self.circle, lambda x, y: x >= 0 and y >= 0
        )

        # Create line for additional testing
        self.line = PolynomialCurve(x + y - 1, variables=(x, y))
        self.line_segment = TrimmedImplicitCurve(
            self.line, lambda x, y: x >= 0 and x <= 1
        )
    ...
    def test_contains_right_half_circle(self):
        ...
        test_cases = [
            ...
            (
                ...
                1 / math.sqrt(2),
                False,
                "Point on left arc (outside mask)",
            ),
            (
                -1 / math.sqrt(2),
                -1 / math.sqrt(2),
    ...
    def test_contains_upper_half_circle(self):
        ...
        test_cases = [
            ...
            (
                ...
                -1 / math.sqrt(2),
                False,
                "Point on lower arc (outside mask)",
            ),
            (
                -1 / math.sqrt(2),
                -1 / math.sqrt(2),
    ...
    def test_contains_first_quadrant_circle(self):
        ...
        test_cases = [
            ...
            (
                ...
                1 / math.sqrt(2),
                False,
                "Point on second quadrant arc",
            ),
            (
                1 / math.sqrt(2),
                -1 / math.sqrt(2),
                False,
                "Point on fourth quadrant arc",
            ),
            (
                -1 / math.sqrt(2),
                -1 / math.sqrt(2),
                False,
                "Point on third quadrant arc",
            ),
        ]
    ...
    def test_contains_tolerance_handling(self):
        ...
        tolerance_cases = [
            ...
            (
                ...
                1.0,
                False,
                "Point very close to boundary but outside right half (x < 0)",
            ),
        ]
...
class TestTrimmedImplicitCurveComplexMasks:
    ...
    def setup_method(self):
        ...
        self.angular_mask = (
            lambda x, y: np.arctan2(y, x) >= 0 and np.arctan2(y, x) <= np.pi / 2
        )  # First quadrant by angle
        self.distance_mask = (
            lambda x, y: abs(x - 0.5) + abs(y - 0.5) <= 0.5
        )  # Diamond region
...
class TestTrimmedImplicitCurveExtensive:
    ...
    def test_bounding_box_scenarios(self):
        ...
        curve1 = TrimmedImplicitCurve(
            base_circle, mask, xmin=-0.5, xmax=0.5, ymin=-0.8, ymax=0.8
        )
        assert curve1.bounding_box() == (-0.5, 0.5, -0.8, 0.8)

        # Case 2: Derived from endpoints
        curve2 = TrimmedImplicitCurve(
            base_circle, mask, endpoints=[(0.0, 1.0), (1.0, 0.0)]
        )
    ...
    def test_serialization_round_trip(self):
        ...
        assert (
            reconstructed_a.contains(0.2, 0.0, tolerance=0.5) is False
        )  # outside xmin=0.5
        ...
        assert (
            reconstructed_b.contains(-1.0, 0.0) is True
        )  # Pass-through accepts left half on base curve
    ...
    def test_vectorized_bounds_vs_fallback(self):
        ...
        curve_fast = TrimmedImplicitCurve(
            base_circle, mask, xmin=0.0, xmax=1.0, ymin=0.0, ymax=1.0
        )
