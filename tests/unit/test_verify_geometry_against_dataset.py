...
DB_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "curves.db")
...
@pytest.mark.skipif(not os.path.exists(DB_PATH), reason="Database curves.db not generated yet")
def test_verify_endpoints():
    ...
    for row in curves_data:
        ...
        assert len(geom_endpoints) == len(db_endpoints), f"Curve #{c_id} ({c_type}) endpoint count mismatch"
        ...
        for db_pt, geom_pt in zip(sorted_db, sorted_geom):
            assert abs(db_pt[0] - geom_pt[0]) < 1e-4, f"Curve #{c_id} ({c_type}) endpoint X mismatch"
            assert abs(db_pt[1] - geom_pt[1]) < 1e-4, f"Curve #{c_id} ({c_type}) endpoint Y mismatch"
...
def test_verify_intersections():
        ...
        if relation_type == "NEAR_MISS":
            ...
            try:
                ...
                if true_gap >= 1e-4 * max_scale:
                    assert len(geom_pts_sorted) == 0, f"Curves {id_a} ∩ {id_b} is a NEAR_MISS with gap {true_gap} >= tol {1e-4 * max_scale} but returned intersections: {geom_pts_sorted}"
            except Exception as e:
                # Fallback to standard check if parsing fails
                assert len(geom_pts_sorted) == 0, f"Curves {id_a} ∩ {id_b} is a NEAR_MISS but returned false-positive intersections: {geom_pts_sorted}"
            
        elif relation_type == "TANGENT":
            tangent_pairs += 1
            # Tangents must return exactly the single point touch point
            assert len(geom_pts_sorted) >= 1, f"Tangent curves {id_a} ∩ {id_b} failed to return intersection point"
            # Verify close proximity
            min_dist = min(math.sqrt((gp[0]-dp[0])**2 + (gp[1]-dp[1])**2) for gp in geom_pts_sorted for dp in db_pts_sorted)
            assert min_dist < 1e-2 * max_scale, f"Tangent curve {id_a} ∩ {id_b} intersection point mismatch"
            ...
            for db_pt in db_pts_sorted:
                ...
                assert distances, f"Curves {id_a} ({type_map[id_a]}) ∩ {id_b} ({type_map[id_b]}) intersection failed to return points"
                min_dist = min(distances)
                assert min_dist < 1e-2 * max_scale, f"Curves {id_a} ({type_map[id_a]}) ∩ {id_b} ({type_map[id_b]}) intersection point {db_pt} missing or mismatched"
    ...
    print(f"Successfully verified {verified_pairs} standard pairs, {near_miss_pairs} near-misses, and {tangent_pairs} tangent touches.")
