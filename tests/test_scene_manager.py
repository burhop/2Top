...
class TestDependencyManagement:
    ...
    def test_register_dependency_with_description(self):
        """Test registering dependency relationship with optional description."""
        scene = SceneManager()
        obj1 = MockAnimatableObject("source")
        obj2 = MockAnimatableObject("dependent")
        ...
        desc_text = "Radius binds to source.radius * 0.5"
        scene.register_dependency("dependent", "source", description=desc_text)

        assert "dependent" in scene._dependency_descriptions
        assert scene._dependency_descriptions["dependent"]["source"] == desc_text

        # Test serialization of dependency descriptions
        ...
        try:
            ...
            # Load in a fresh scene manager
            scene2 = SceneManager()
            scene2.load_scene(filename)

            assert "dependent" in scene2._dependency_descriptions
            assert scene2._dependency_descriptions["dependent"]["source"] == desc_text

            # Test cleanup on removal
            scene.remove_object("source")
            assert "dependent" not in scene._dependency_descriptions
...
class TestAnimationSystem:
    ...
    def test_create_multi_parameter_animation_mismatched_lengths_fails(self):
        ...
        with pytest.raises(
            ValueError, match="All animations must have the same number of values"
        ):
