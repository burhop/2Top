...
class TestPersistenceRegressions:
    ...
    def test_save_load_with_complex_data(self):
        ...
        obj1 = ComplexAnimatableObject(
            "complex1", x=np.pi, y=np.e, radius=1.414, angle=2 * np.pi / 3, visible=True
        )
        obj2 = ComplexAnimatableObject(
            "complex2", x=-1.5, y=0.0, scale=0.707, angle=-np.pi / 4, visible=False
        )
