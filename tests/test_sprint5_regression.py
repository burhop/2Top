...
class TestTrimmedImplicitCurveInterfaceConsistency:
    ...
    def setup_method(self):
        ...
        self.procedural = ProceduralCurve(
            lambda x, y: x**2 + y**2 - 1, variables=(x, y)
        )
        ...
        self.trimmed_super = TrimmedImplicitCurve(
            self.superellipse, lambda x, y: x >= 0 and y >= 0
        )
    ...
    def test_plotting_interface(self):
        ...
        for curve in self.all_trimmed:
            ...
            try:
                ...
                plt.close(fig)
                pytest.fail(
                    f"Plotting failed for {type(curve.base_curve).__name__}: {e}"
                )
...
class TestCompositeCurveInterfaceConsistency:
    ...
    def setup_method(self):
        ...
        self.all_composites = [
            self.simple_composite,
            self.mixed_composite,
            self.single_segment_composite,
        ]
...
class TestCrossClassCompatibility:
    ...
    def setup_method(self):
        ...
        # Create trimmed versions
        self.trimmed_conic = TrimmedImplicitCurve(self.conic, lambda x, y: x >= 0)
        self.trimmed_rfunction = TrimmedImplicitCurve(
            self.rfunction, lambda x, y: y >= 0
        )

        # Create composite
        self.composite = CompositeCurve(
            [self.trimmed_conic, TrimmedImplicitCurve(self.poly, lambda x, y: x <= 0)]
        )
    ...
    def test_nested_trimmed_curves(self):
        ...
        trimmed_complex = TrimmedImplicitCurve(
            complex_base, lambda x, y: x >= 0 and y >= 0
        )
    ...
    def test_serialization_cross_compatibility(self):
        ...
        curves_to_test = [self.trimmed_conic, self.trimmed_rfunction, self.composite]
