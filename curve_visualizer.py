...
class CurveVisualizerApp:
    ...
    def setup_controls(self, parent):
        ...
        title_label = ttk.Label(
            parent, text="Curve Controls", font=("Arial", 14, "bold")
        )
        ...
        for curve_type in curve_types:
            ttk.Radiobutton(
                type_frame, text=curve_type, variable=self.curve_type, value=curve_type
        ...
        ttk.Button(
            button_frame, text="Remove Selected", command=self.remove_curve
        ).pack(fill=tk.X, pady=2)
        ttk.Button(button_frame, text="Clear All", command=self.clear_curves).pack(
            fill=tk.X, pady=2
        )
        ttk.Button(button_frame, text="Refresh Plot", command=self.update_plot).pack(
            fill=tk.X, pady=2
        )
        ...
        # Plot range
        ttk.Label(settings_frame, text="Plot Range:").pack(
            ...
            if curve_type == "Circle":
                ...
                if len(self.curves) >= 2:
                    ...
                    messagebox.showwarning(
                        "Warning", "Need at least 2 curves for union"
                    )
                    return
                ...
                if len(self.curves) >= 2:
                    ...
                    messagebox.showwarning(
                        "Warning", "Need at least 2 curves for intersection"
                    )
                    return
    ...
    def update_plot(self):
        ...
        for i, (name, curve) in enumerate(self.curves):
            try:
                ...
                self.ax.contourf(
                    X, Y, Z, levels=[-1000, 0, 1000], colors=[color, "white"], alpha=0.1
                )
