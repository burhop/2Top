...
def test_overlapping_curves():
    ...
    intersections = find_curve_intersections(
        circle1, circle2, search_range=2.0, tolerance=1e-8
    )
    print("  Circle 1: x² + y² = 1")
    print("  Circle 2: x² + y² = 1 (identical)")
    ...
    intersections = find_curve_intersections(
        quarter_circle, half_circle, search_range=2.0, tolerance=1e-8
    )
    print("  Quarter Circle: First quadrant of x² + y² = 1")
    print("  Half Circle: Right half of x² + y² = 1")
    print(f"  Found {len(intersections)} intersection(s)")
    ...
    intersections = find_curve_intersections(
        segment1, segment2, search_range=3.0, tolerance=1e-8
    )
    print("  Segment 1: y = 0, x ∈ [-1, 1]")
    print("  Segment 2: y = 0, x ∈ [0, 2]")
    print("  Expected: Overlap from x = 0 to x = 1")
    print(f"  Found {len(intersections)} intersection(s)")
    for i, (px, py) in enumerate(intersections):
        print(f"    {i + 1}. ({px:7.4f}, {py:7.4f})")
    ...
    intersections = find_curve_intersections(
        circle_a, circle_b, search_range=2.0, tolerance=1e-8
    )
    print("  Circle A: x² + y² = 1")
    print("  Circle B: (x-1)² + y² = 1")
    print("  Expected: 2 discrete intersection points")
    print(f"  Found {len(intersections)} intersection(s)")
    for i, (px, py) in enumerate(intersections):
        print(f"    {i + 1}. ({px:7.4f}, {py:7.4f})")
    ...
    for idx, (curve1, curve2, title, _) in enumerate(test_cases):
        ...
        intersections = find_curve_intersections(
            curve1, curve2, search_range=plot_range, tolerance=1e-8
        )
        ...
        try:
            ...
            ax.contour(
                X, Y, Z1_masked, levels=[0], colors=["blue"], linewidths=3, alpha=0.7
            )
            ax.contour(
                X, Y, Z2_masked, levels=[0], colors=["red"], linewidths=3, alpha=0.7
            )
    ...
    print("\n💡 RECOMMENDATION:")
    print(
        "For most geometric applications, discrete intersection points are most useful."
    )
    print(
        "Overlapping segments should be handled as a special case in higher-level logic."
    )
