...
def test_precise_intersections():
    ...
    found_intersections = find_curve_intersections(
        parabola, line, search_range=3.0, grid_resolution=400, tolerance=1e-8
    )
    ...
    for i, (px, py) in enumerate(found_intersections):
        ...
        print(
            f"      Check y = x²: {py:.6f} vs {px**2:.6f} (diff: {abs(py - px**2):.8f})"
        )
    ...
    for analytical in analytical_solutions:
        ...
        for found in found_intersections:
            distance = np.sqrt(
                (found[0] - analytical[0]) ** 2 + (found[1] - analytical[1]) ** 2
            )
        ...
        if closest_found:
            print(
                f"    Analytical {analytical} → Found {closest_found} (distance: {min_distance:.6f})"
            )
        else:
    ...
    for px, py in analytical_solutions:
        ax1.plot(
            ...
            markeredgewidth=2,
            label="Analytical" if px == analytical_solutions[0][0] else "",
        )
    ...
    zoom_range = 2
    ax2.contour(
        X, Y, Z_parabola, levels=[0], colors=["blue"], linewidths=2, label="y = x²"
    )
    ...
    for i, (px, py) in enumerate(analytical_solutions):
        ax2.plot(
            px, py, "go", markersize=12, markeredgecolor="black", markeredgewidth=2
        )
        ax2.annotate(
            f"Analytical\n({px}, {py})",
            xy=(px, py),
    ...
    if found_count == expected_count:
        ...
        for analytical in analytical_solutions:
            closest_found = min(
                ...
                key=lambda p: np.sqrt(
                    (p[0] - analytical[0]) ** 2 + (p[1] - analytical[1]) ** 2
                ),
            )
            error = np.sqrt(
                (closest_found[0] - analytical[0]) ** 2
                + (closest_found[1] - analytical[1]) ** 2
            )
        ...
        print(
            "  ⚠️  WARNING: More intersections found than expected (possible false positives)"
        )
    else:
        print(
            "  ❌ ERROR: Fewer intersections found than expected (missing intersections)"
        )
