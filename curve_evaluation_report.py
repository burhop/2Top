...
def comprehensive_evaluation_report():
    ...
    test_curve(
        "ConicSection",
        circle,
        "Circle basic evaluation",
        lambda: abs(circle.evaluate(0, 0) + 1) < 1e-10,
    )
    test_curve(
        "ConicSection",
        circle,
        "Circle boundary evaluation",
    ...
    test_curve(
        ...
        union_curve,
        "Union evaluation",
        lambda: union_curve.evaluate(0, 0) < 0,
    )  # Should be inside
    ...
    test_curve(
        ...
        lambda: intersect_curve.evaluate(0.5, 0) < 0,
    )  # Should be in overlap
    ...
    test_curve(
        ...
        lambda: diff_curve.evaluate(-0.5, 0) < 0,
    )  # Should be in c1 but not c2
    ...
    test_curve(
        ...
        blend_curve,
        "Blend evaluation",
        lambda: blend_curve.evaluate(0, 0) < 0,
    )  # Should be inside
    ...
    test_curve("CompositeCurve", square, "Square creation", lambda: square.is_closed())
    test_curve(
        "CompositeCurve",
        square,
        "Square corner evaluation",
        lambda: abs(square.evaluate(1, 1)) < 1e-6,
    )
    test_curve(
        "CompositeCurve",
        square,
        "Square inside evaluation",
    ...
    test_curve(
        "CompositeCurve",
        circle_quarters,
        "Circle quarters creation",
        lambda: circle_quarters.is_closed(),
    )
    test_curve(
        "CompositeCurve",
        circle_quarters,
        "Circle quarters evaluation",
        lambda: abs(circle_quarters.evaluate(1, 0)) < 0.1,
    )  # Looser tolerance for composite
    ...
    test_curve(
        "AreaRegion",
        region,
        "Region containment (inside)",
        lambda: region.contains(0, 0),
    )
    test_curve(
        "AreaRegion",
        region,
        "Region containment (outside)",
    ...
    test_curve(
        "AreaRegion",
        region_with_hole,
        "Region with hole (outside hole)",
        lambda: region_with_hole.contains(1, 1),
    )
    test_curve(
        "AreaRegion",
        region_with_hole,
        "Region with hole (inside hole)",
    ...
    test_curve(
        "ConicSection",
        circle,
        f"Large array performance ({perf_time:.3f}s)",
        lambda: perf_ok,
    )
    ...
    for curve_type, data in results.items():
        ...
        if total > 0:
            ...
            print(
                f"{status} {curve_type:15} {passed:2d}/{total:2d} ({percentage:5.1f}%)"
            )
    ...
    if results["Superellipse"]["passed"] < results["Superellipse"]["total"]:
        recommendations.append(
            "⭐ Review Superellipse implementation for edge cases (extreme n values)"
        )

    if results["CompositeCurve"]["passed"] < results["CompositeCurve"]["total"]:
        recommendations.append(
            "🔗 Check CompositeCurve evaluation method for numerical precision"
        )

    if results["RFunctionCurve"]["passed"] < results["RFunctionCurve"]["total"]:
        recommendations.append(
            "🔗 Verify R-function implementations match mathematical definitions"
        )
