...
def analyze_intersection_concepts():
    ...
    for px, py in test_points:
        ...
        print(
            f"    ({px:4.1f}, {py:4.1f}): circle={circle_val:6.2f}, line={line_val:6.2f}, "
    ...
    fig.suptitle(
        "Curve Intersections vs R-Function Operations", fontsize=14, fontweight="bold"
    )
    ...
    ax1.contour(
        X, Y, Z_circle, levels=[0], colors=["blue"], linewidths=2, label="Circle"
    )
    ...
    for px, py in intersection_points:
        ax1.plot(
            ...
            "ko",
            markersize=8,
            label="Intersection" if px == intersection_points[0][0] else "",
        )
    ...
    ax3.contourf(
        X, Y, Z_union, levels=[-1000, 0, 1000], colors=["plum", "white"], alpha=0.5
    )
