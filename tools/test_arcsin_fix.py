...
import sqlite3, json, sys, time, warnings
sys.path.append('.')
from tools.compare_solutions_suite import reconstruct_db_curve, translate_curve, match_points
...
for c_a_id, c_b_id, desc in test_cases:
    cursor.execute('SELECT id, group_id, equation, type, scale, bounds_xmin, bounds_xmax, bounds_ymin, bounds_ymax, endpoints, local_tolerance FROM curves WHERE id IN (?, ?)', (c_a_id, c_b_id))
    rows = {r[0]: r for r in cursor.fetchall()}
    c_a = reconstruct_db_curve(rows[c_a_id])
    c_b = reconstruct_db_curve(rows[c_b_id])
    cursor.execute('SELECT intersections FROM intersections WHERE (curve_a_id=? AND curve_b_id=?) OR (curve_a_id=? AND curve_b_id=?)', (c_a_id, c_b_id, c_b_id, c_a_id))
    ...
    with warnings.catch_warnings():
        ...
        pts = find_curve_intersections(ca_t, cb_t, search_range=search_range, grid_resolution=500, tolerance=1e-4*max_scale)
    ...
    if passed:
        passed_all += 1
    print(f'{status} {desc}: expected={len(expected_pts)}, found={len(found)}, t={elapsed:.3f}s')
