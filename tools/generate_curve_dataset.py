...
TOTAL_GROUPS = int(
    os.environ.get("TOTAL_GROUPS", 100000)
)  # Default to 100,000 groups, but configurable
CURVES_PER_GROUP = 10
...
def intersect_circle_circle_analytical(cx1, cy1, r1, cx2, cy2, r2):
    ...
    return [(x2_temp + rx, y2_temp + ry), (x2_temp - rx, y2_temp - ry)]
...
def generate_spatial_group(args):
    ...
    if group_type == 0:
        ...
        endpoints = []
        k_values = sorted(
            range(-10, 11), key=lambda k: abs((k * math.pi - c_param) / b_param)
        )
                    ...
                    for cand in candidates:
                        ...
                        try:
                            sol, infodict, ier, mesg = fsolve(
                                system, cand, full_output=True, xtol=tol
                            )
                            if ier == 1:
                                # Verify the residual is actually small
                                residual = system(sol)
                                if (
                                    math.sqrt(residual[0] ** 2 + residual[1] ** 2)
                                    < 1e-6 * scale
                                ):
...
def writer_process(queue, db_path, total_groups):
    ...
    while True:
        ...
        if intersections_records:
            cursor.executemany(
                "INSERT INTO intersections VALUES (?, ?, ?, ?);", intersections_records
            )
        ...
        if written_groups % BATCH_SIZE == 0:
            conn.commit()
            sys.stdout.write(
                f"\rProgress: {written_groups}/{total_groups} groups generated..."
            )
            sys.stdout.flush()
