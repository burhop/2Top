...
def main():
    ...
    if periodic_command and periodic_interval:
        print(
            f"Running periodic command: '{periodic_command}' every {periodic_interval} seconds.\n"
        )
        ...
        while True:
                ...
                try:
                    ...
                    if cmd_ts > last_cmd_timestamp:
                        ...
                        if p_interval is not None:
                            periodic_interval = (
                                float(p_interval) if p_interval > 0 else None
                            )
                            periodic_command = cmd_str
                            last_periodic_run = 0.0  # trigger immediately
                            print(
                                f"Updated periodic run: '{periodic_command}' every {periodic_interval}s"
                            )
                        else:
                            # Single-shot run
                            periodic_interval = None
            ...
            # 3. Check for file changes (only when not running a periodic command)
                ...
                if changed_files:
                    ...
                    time.sleep(0.5)
                    auto_cmd = (
                        "pytest tests/unit/test_verify_geometry_against_dataset.py"
                    )
