...
def main():
    ...
    for row_id in target_rows:
        ...
        print(
            f"  Expected count: {res['expected_count']}, Analytical: {res['analytical_count']} (Pass: {res['analytical_pass']}, Msg: {res['analytical_msg']})"
        )
        print(
            f"  Graphics: {res['graphics_count']} (Pass: {res['graphics_pass']}, Msg: {res['graphics_msg']})"
        )
