...
def print_statistics(conn):
    ...
    print("\nCurves by Type:")
    cursor.execute(
        "SELECT type, COUNT(*) FROM curves GROUP BY type ORDER BY COUNT(*) DESC;"
    )
    ...
    cursor.execute(
        "SELECT relation_type, COUNT(*) FROM intersections GROUP BY relation_type ORDER BY COUNT(*) DESC;"
    )
    for row in cursor.fetchall():
        print(f"  - {row[0]:<20}: {row[1]:,}")
...
def query_group(conn, group_id):
    ...
    # Fetch curves
    cursor.execute(
        "SELECT id, type, equation, endpoints FROM curves WHERE group_id = ?;",
        (group_id,),
    )
    ...
    for inter in intersections:
        ...
        print(
            f"  Curves {inter[0]} intersect {inter[1]} | Type: {inter[3]} | Intersections found: {len(pts)}"
        )
...
def main():
    if not os.path.exists(DB_PATH):
        print(
            f"Error: curves.db does not exist at '{DB_PATH}'. Please run tools/generate_curve_dataset.py first."
        )
        sys.exit(1)
