...
def demo_trimmed_curves():
    ...
    print(
        f"Values match:  {abs(base_val - right_val) < 1e-10 and abs(base_val - upper_val) < 1e-10}"
    )
...
def demo_composite_curves():
    ...
    print(
        f"Horizontal split (upper + lower): {len(horizontal_split.segments)} segments"
    )
    ...
    for px, py in test_points:
        ...
        print(
            f"({px:5.3f}, {py:5.3f}) | {str(vert_contains):8} | {str(horiz_contains):10}"
        )
...
def demo_constructive_geometry():
    ...
    # Create base curves
    circle = ConicSection(x**2 + y**2 - 1, variables=(x, y))
    square_poly = PolynomialCurve(
        x**2 + y**2 - 0.5, variables=(x, y)
    )  # Smaller circle as square approximation
...
def demo_serialization():
    ...
    print(
        "   ⚠️  Manual reconstruction of mask functions required for full functionality"
    )
...
def main():
    ...
    try:
        ...
        print(
            "  Superellipse, ProceduralCurve, RFunctionCurve, TrimmedImplicitCurve, CompositeCurve"
        )
        print("• 2,000+ total test cases across all sprints")
        print(
            "• Complete functionality: evaluation, gradients, normals, plotting, serialization"
        )
