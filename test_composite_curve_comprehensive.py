...
class TestCompositeCurveBasics:
    ...
    def test_bounding_box(self):
        ...
        seg1 = TrimmedImplicitCurve(
            self.line1, lambda x, y: 0 <= x <= 1, xmin=0, xmax=1, ymin=-0.1, ymax=0.1
        )
        seg2 = TrimmedImplicitCurve(
            self.line2, lambda x, y: 0 <= y <= 1, xmin=0.9, xmax=1.1, ymin=0, ymax=1
        )
        composite = CompositeCurve([seg1, seg2])
...
class TestCompositeCurveContinuity:
    ...
    def test_continuity_validation_with_endpoints(self):
        ...
        seg1 = TrimmedImplicitCurve(
            self.line1, lambda x, y: 0 <= x <= 1, endpoints=[(0, 0), (1, 0)]
        )
        seg2 = TrimmedImplicitCurve(
            self.line2, lambda x, y: 0 <= y <= 1, endpoints=[(1, 0), (1, 1)]
        )
    ...
    def test_continuity_validation_fails_with_gap(self):
        ...
        seg2 = TrimmedImplicitCurve(
            self.line2, lambda x, y: 0 <= y <= 1, endpoints=[(2, 1), (2, 2)]
        )  # Gap!
...
class TestCompositeCurveComplexShapes:
    ...
    def test_create_triangle(self):
        ...
        line2 = PolynomialCurve(
            self.y + 1.5 * self.x - 1, (self.x, self.y)
        )  # Right edge
        line3 = PolynomialCurve(
            self.y - 1.5 * self.x - 1, (self.x, self.y)
        )  # Left edge

        seg1 = TrimmedImplicitCurve(
            line1, lambda x, y: -1 <= x <= 1, endpoints=[(-1, -0.5), (1, -0.5)]
        )
        seg2 = TrimmedImplicitCurve(
            line2, lambda x, y: 0 <= x <= 1, endpoints=[(1, -0.5), (0, 1)]
        )
        seg3 = TrimmedImplicitCurve(
            line3, lambda x, y: -1 <= x <= 0, endpoints=[(0, 1), (-1, -0.5)]
        )
    ...
    def test_create_L_shape(self):
        ...
        seg1 = TrimmedImplicitCurve(
            line1, lambda x, y: -1 <= y <= 0, endpoints=[(-0.5, -1), (-0.5, 0)]
        )
        seg2 = TrimmedImplicitCurve(
            line2, lambda x, y: -0.5 <= x <= 0.5, endpoints=[(-0.5, -1), (0.5, -1)]
        )
