...
def demo_sharp_operations():
    ...
    print(
        f"{'Point':<20} {'Union':<8} {'Intersect':<10} {'Difference':<10} {'Description'}"
    )
    print("-" * 70)
    ...
    for x, y, desc in test_points:
        ...
        print(
            f"({x:4.1f}, {y:4.1f}){'':<8} {union_val:7.3f}  {intersect_val:9.3f}  {diff_val:9.3f}  {desc}"
        )
...
def demo_smooth_blending():
    ...
    print(
        f"Smooth blend (α=0.1) at (0.5, 0): {blend_curves[0.1].evaluate(0.5, 0.0):.6f}"
    )
    print(
        f"Smooth blend (α=1.0) at (0.5, 0): {blend_curves[1.0].evaluate(0.5, 0.0):.6f}"
    )
...
def demo_nested_operations():
    ...
    print("\nBase shapes:")
    print("  Circle 1: centered at origin, radius 1")
    print("  Circle 2: centered at (1,0), radius 1")
    print("  Circle 3: centered at (0.5,0.5), radius 0.5")
...
def demo_serialization():
    ...
    for x, y in test_points:
        ...
        print(
            f"({x:4.1f}, {y:4.1f}){'':<4} {orig_val:<12.6f} {rest_val:<12.6f} {diff:<12.2e}"
        )
...
def demo_vectorized_operations():
    ...
    for i, x in enumerate(x_vals):
        ...
        print(
            f"{x:<6.1f} {circle_results[i]:<10.3f} {square_results[i]:<10.3f} "
...
def demo_interface_consistency():
    ...
    print("All curve types inherit from ImplicitCurve:")
