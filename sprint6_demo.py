...
def demo_region_with_holes():
    ...
    print("Outer boundary: 6x6 square (area = 36)")
    print("Hole 1: 1x1 square (area = 1)")
    print("Hole 2: 1x1 square (area = 1)")
    print("Expected total area: 36 - 1 - 1 = 34")
...
def demo_serialization():
    ...
    containment_match = original.contains(0, 0) == reconstructed.contains(
        0, 0
    ) and original.contains(1.5, 0) == reconstructed.contains(1.5, 0)
...
def demo_performance():
    ...
    n_tests = 1000
    test_points = [
        (np.random.uniform(-2, 2), np.random.uniform(-2, 2)) for _ in range(n_tests)
    ]
    ...
    print("Containment testing performance:")
    ...
    print("  100 area calculations")
    print(f"  Total time: {end_time - start_time:.4f} seconds")
    print(
        f"  Average time per calculation: {(end_time - start_time) / 100 * 1000:.3f} ms"
    )
