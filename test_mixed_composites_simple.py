...
def test_basic_mixed_composite():
    ...
    circle_right = TrimmedImplicitCurve(
        circle, lambda x_val, y_val: x_val >= 0, endpoints=[(0, -1), (0, 1)]
    )
    ...
    for i, (px, py) in enumerate(test_points):
        ...
        ax.annotate(
            f"P{i + 1}({px}, {py})", (px, py), xytext=(5, 5), textcoords="offset points"
        )
...
def test_factory_functions():
    ...
    try:
        ...
        print(
            f"  ✅ Ellipse-parabola hybrid created with {len(egg_shape.segments)} segments"
        )
    except Exception as e:
    ...
    try:
        ...
        print(
            f"  ✅ Superellipse-circle hybrid created with {len(rounded_square.segments)} segments"
        )
    except Exception as e:
