...
def list_periodic_tests():
    ...
    try:
        tests_list = []
        for test_id, config in sorted(TEST_CONFIGS.items(), key=lambda x: [int(c) for c in x[0].split('.')]):
            tests_list.append({
                'test_id': test_id,
                'name': config['name'],
                'eq_a': config['eq_a'],
...
def run_periodic_test():
    ...
    try:
        ...
        if not test_id or test_id not in TEST_CONFIGS:
            return jsonify({'success': False, 'error': f"Test ID '{test_id}' not found"}), 400
