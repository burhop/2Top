...
def test_triangle_with_endpoints():
    ...
    for line, name, vertices in lines_vertices:
        ...
        for i, vertex in enumerate(vertices):
            ...
            vertex_name = (
                ["A", "B", "C"][["A", "B", "C"].index(name[0]) + i]
                if i == 0
                else ["A", "B", "C"][["A", "B", "C"].index(name[1])]
            )
            print(
                f"  Vertex {vertex_name}{vertex}: f = {val:.10f} {'✓' if abs(val) < 1e-10 else '✗'}"
            )
    ...
    def mask_BC(px, py):
        ...
        in_x_range = (
            min(vertex_B[0], vertex_C[0]) <= px <= max(vertex_B[0], vertex_C[0])
        )
        in_y_range = (
            min(vertex_B[1], vertex_C[1]) <= py <= max(vertex_B[1], vertex_C[1])
        )
    ...
    def mask_CA(px, py):
        ...
        on_line = abs(3 * px - 2 * py + 4) < 0.1
        in_x_range = (
            min(vertex_C[0], vertex_A[0]) <= px <= max(vertex_C[0], vertex_A[0])
        )
        in_y_range = (
            min(vertex_C[1], vertex_A[1]) <= py <= max(vertex_C[1], vertex_A[1])
        )
        return on_line and in_x_range and in_y_range
        ...
        try:
            ...
            for px, py, description in test_points:
                try:
                    ...
                    is_boundary = triangle_area.contains(
                        px, py, region_containment=False
                    )

                    status = (
                        "INSIDE"
                        if is_inside
                        else ("BOUNDARY" if is_boundary else "OUTSIDE")
                    )
                    print(
                        f"  Point ({px:4.1f}, {py:4.1f}) [{description:15s}]: {status}"
                    )

                except Exception as e:
                    print(
                        f"  Point ({px:4.1f}, {py:4.1f}) [{description:15s}]: ERROR - {e}"
                    )
    ...
    for (
        ...
        for curr_end in current_endpoints:
            for next_end in next_endpoints:
                distance = np.sqrt(
                    (curr_end[0] - next_end[0]) ** 2 + (curr_end[1] - next_end[1]) ** 2
                )
                if distance <= tolerance:
                    connected = True
                    print(
                        f"  ✓ Connected: {curr_end} ↔ {next_end} (distance: {distance:.10f})"
                    )
                    break
            if connected:
                break
