...
def demo_blended_fields(circle_field, ellipse_field):
    ...
    for px, py in test_points:
        ...
        print(
            f"({px:2}, {py:2})\t\t{add_val:6.2f}\t\t{min_val:6.2f}\t\t{max_val:6.2f}\t\t{mult_val:6.2f}"
        )
    ...
    print(
        f"Blended field serialized: {blended_data['type']}, operation: {blended_data['operation']}"
    )
...
def demo_field_strategies():
    ...
    print(
        f"- Occupancy Strategy: inside={occupancy_strategy.inside_value}, outside={occupancy_strategy.outside_value}"
    )
    ...
    print(
        f"SDF reconstruction successful: {sdf_reconstructed.resolution == sdf_strategy.resolution}"
    )
    print(
        f"Occupancy reconstruction successful: {occ_reconstructed.inside_value == occupancy_strategy.inside_value}"
    )
    ...
    for i, x in enumerate(x_range[::2]):  # Sample every other point
        for j, y in enumerate(y_range[::2]):
            try:
                ...
                print(
                    f"({x:3.1f}, {y:3.1f})\t\t({grad_x:6.3f}, {grad_y:6.3f}) |{magnitude:5.3f}|"
                )
            except Exception as e:
...
def demo_field_visualization():
    ...
    try:
        ...
        # Plot field values
        plt.contourf(
            X, Y, Z, levels=[0, 0.5, 1], colors=["lightblue", "darkblue"], alpha=0.7
        )
