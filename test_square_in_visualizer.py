...
def test_square_in_visualizer_context():
    ...
    try:
        ...
        for px, py in test_points:
            ...
            contains_region = square.contains(
                px, py, tolerance=0.1, region_containment=True
            )
            print(
                f"  Point ({px:4.1f}, {py:4.1f}): eval={val:6.3f}, boundary={contains_boundary}, region={contains_region}"
            )
        ...
        contour_sets = square.plot(
            ax=ax, x_range=(-2, 2), y_range=(-2, 2), resolution=1000
        )
        ...
        for i, (px, py) in enumerate(test_points):
            ...
            ax.annotate(
                f"P{i + 1}", (px, py), xytext=(5, 5), textcoords="offset points"
            )
...
def test_square_segments_individually():
    ...
    for i, (segment, name) in enumerate(zip(square.segments, segment_names)):
        ...
        for px, py in test_points:
            ...
            print(
                f"    Point ({px:4.1f}, {py:4.1f}): contains={contains}, eval={eval_val:6.3f}, mask={mask_val}"
            )
