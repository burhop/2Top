...
def test_continuity_enforcement():
    ...
    for name, creator in continuous_shapes:
        ...
        try:
            ...
            if len(shape.segments) > 1:
                ...
                num_checks = len(shape.segments) if is_closed else len(shape.segments) - 1
                ...
                if is_truly_continuous:
                    ...
                    print(f"  ✅ {name}: {len(shape.segments)} segments, {closed_status}, max gap: {max_gap:.2e}")
                else:
                    print(f"  ❌ {name}: {len(shape.segments)} segments, max gap: {max_gap:.2e} (TOO LARGE!)")
    ...
    try:
        ...
        seg2 = TrimmedImplicitCurve(line2, lambda x, y: 2 <= x <= 3, endpoints=[(2, 1), (3, 1)])  # BIG GAP!
        
        # This should fail
        discontinuous = CompositeCurve([seg1, seg2], validate_continuity=True)
        print(f"  ❌ ERROR: Discontinuous curve was accepted! This should have failed.")
        all_continuous = False
...
def test_specific_continuity_cases():
    ...
    try:
        ...
        if shared_point:
            ...
            print(f"  ❌ T-shape segments don't connect properly")
    ...
    try:
        ...
        if not connection_found:
            print(f"  ❌ Zigzag segments don't connect properly")
...
if __name__ == "__main__":
    ...
    if continuous:
        ...
        print("CompositeCurve now properly represents 'continuous paths with connectivity checking'")
    else:
