...
def test_intersection_performance():
    ...
    test_cases = [
        ...
        (
            TrimmedImplicitCurve(
                ConicSection(x**2 + y**2 - 1, (x, y)), lambda x, y: x >= 0
            ),
    ...
    for curve1, curve2, name in test_cases:
        ...
        print(
            f"  Fast mode:  {len(intersections_fast)} intersections in {fast_time:.3f}s"
        )
        print(
            f"  Slow mode:  {len(intersections_slow)} intersections in {slow_time:.3f}s"
        )
        ...
        if intersections_fast:
            print(
                f"  Points (fast): {intersections_fast[:3]}{'...' if len(intersections_fast) > 3 else ''}"
            )
        if intersections_slow:
            print(
                f"  Points (slow): {intersections_slow[:3]}{'...' if len(intersections_slow) > 3 else ''}"
            )

    print("\n✅ Performance test complete!")
    print("💡 Recommendation: Use detect_overlap=False for interactive applications")
