...
def demo_procedural_curves():
    ...
    curves = {
        name: ProceduralCurve(func, name=name) for name, func in functions.items()
    }
...
def demo_gradient_comparison():
    ...
    for point in test_points:
        ...
        # Check accuracy
        symbolic_error = max(
            abs(symbolic_grad[0] - analytical[0]), abs(symbolic_grad[1] - analytical[1])
        )
        super_error = max(
            abs(super_grad[0] - analytical[0]), abs(super_grad[1] - analytical[1])
        )
        numerical_error = max(
            abs(numerical_grad[0] - analytical[0]),
            abs(numerical_grad[1] - analytical[1]),
        )

        print(
            f"  Errors: Symbolic={symbolic_error:.2e}, Super={super_error:.2e}, Numerical={numerical_error:.2e}"
        )
...
def demo_serialization_behavior():
    ...
    print(
        f"  Parameters: a={square_data['a']}, b={square_data['b']}, n={square_data['n']}"
    )
    ...
    print("    ✓ Serialization preserves functionality")
...
def demo_vectorized_operations():
    ...
    print("ProceduralCurve gradients:")
...
def demo_specialized_features():
    ...
    for name, func in functions:
        try:
            ...
            print(
                f"  {name:12}: f(0,0)={val_origin:6.3f}, f(1,0)={val_unit:6.3f}, ∇f(1,0)=({grad_unit[0]:5.2f},{grad_unit[1]:5.2f})"
            )
